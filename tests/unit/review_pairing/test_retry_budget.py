...
class TestComputeSequentialWorstCase:
    def test_matches_hand_derived_formula_for_qwen3_review_b(self) -> None:
        ...
        assert result.worst_case_seconds == 2 * 300.0 + 2.0
...
class TestAssertBudgetWithinCeiling:
    def test_red_undersized_ceiling_still_violates_invariant(self) -> None:
        """RED case, kept live against the current registry (not frozen to
        the original OMN-15066 numbers): an insufficient job ceiling must
        still be caught. The original incident this test documented (the
        hostile-reviewer job's 35-minute/2100s ceiling vs. qwen3-review-b's
        old 1200s-timeout worst case of 2414s, OMN-15066, jobs
        89616541439/89616551741) no longer reproduces verbatim -- OMN-16407
    ...
    def test_green_new_hostile_reviewer_ceiling_holds(self) -> None:
        ...
        (1096s, post-OMN-16407) plus a realistic setup-overhead buffer.
        """
