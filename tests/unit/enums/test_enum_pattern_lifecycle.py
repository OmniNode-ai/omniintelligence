...
class TestEnumPatternLifecycleStatusTransitions:
    ...
    def test_candidate_can_transition_to_provisional(self) -> None:
        """CANDIDATE can transition to PROVISIONAL (initial quality threshold met)."""
        status = EnumPatternLifecycleStatus.CANDIDATE
        assert status.can_transition_to(EnumPatternLifecycleStatus.PROVISIONAL) is True

    def test_candidate_can_transition_to_deprecated(self) -> None:
        """CANDIDATE can transition to DEPRECATED (failure or manual deprecation)."""
        status = EnumPatternLifecycleStatus.CANDIDATE
        assert status.can_transition_to(EnumPatternLifecycleStatus.DEPRECATED) is True

    def test_provisional_can_transition_to_validated(self) -> None:
        """PROVISIONAL can transition to VALIDATED (temporal stability + quality)."""
        status = EnumPatternLifecycleStatus.PROVISIONAL
        assert status.can_transition_to(EnumPatternLifecycleStatus.VALIDATED) is True

    def test_provisional_can_transition_to_deprecated(self) -> None:
        """PROVISIONAL can transition to DEPRECATED (failure streak)."""
        status = EnumPatternLifecycleStatus.PROVISIONAL
        assert status.can_transition_to(EnumPatternLifecycleStatus.DEPRECATED) is True

    def test_validated_can_transition_to_deprecated(self) -> None:
        """VALIDATED can transition to DEPRECATED (pattern no longer recommended)."""
        status = EnumPatternLifecycleStatus.VALIDATED
        assert status.can_transition_to(EnumPatternLifecycleStatus.DEPRECATED) is True
    ...
    def test_candidate_cannot_transition_to_validated(self) -> None:
        """CANDIDATE cannot skip PROVISIONAL to go directly to VALIDATED."""
        status = EnumPatternLifecycleStatus.CANDIDATE
        assert status.can_transition_to(EnumPatternLifecycleStatus.VALIDATED) is False

    def test_deprecated_cannot_transition_to_candidate(self) -> None:
        """DEPRECATED cannot transition back to CANDIDATE (terminal state)."""
        status = EnumPatternLifecycleStatus.DEPRECATED
        assert status.can_transition_to(EnumPatternLifecycleStatus.CANDIDATE) is False

    def test_deprecated_cannot_transition_to_provisional(self) -> None:
        """DEPRECATED cannot transition back to PROVISIONAL (terminal state)."""
        status = EnumPatternLifecycleStatus.DEPRECATED
        assert status.can_transition_to(EnumPatternLifecycleStatus.PROVISIONAL) is False

    def test_deprecated_cannot_transition_to_validated(self) -> None:
        """DEPRECATED cannot transition to VALIDATED (terminal state)."""
        status = EnumPatternLifecycleStatus.DEPRECATED
        assert status.can_transition_to(EnumPatternLifecycleStatus.VALIDATED) is False
    ...
    def test_provisional_cannot_transition_to_candidate(self) -> None:
        """PROVISIONAL cannot regress back to CANDIDATE."""
        status = EnumPatternLifecycleStatus.PROVISIONAL
        assert status.can_transition_to(EnumPatternLifecycleStatus.CANDIDATE) is False

    def test_validated_cannot_transition_to_candidate(self) -> None:
        """VALIDATED cannot regress back to CANDIDATE."""
        status = EnumPatternLifecycleStatus.VALIDATED
        assert status.can_transition_to(EnumPatternLifecycleStatus.CANDIDATE) is False

    def test_validated_cannot_transition_to_provisional(self) -> None:
        """VALIDATED cannot regress back to PROVISIONAL."""
        status = EnumPatternLifecycleStatus.VALIDATED
        assert status.can_transition_to(EnumPatternLifecycleStatus.PROVISIONAL) is False
    ...
    def test_candidate_cannot_transition_to_self(self) -> None:
        """CANDIDATE cannot transition to itself."""
        status = EnumPatternLifecycleStatus.CANDIDATE
        assert status.can_transition_to(EnumPatternLifecycleStatus.CANDIDATE) is False

    def test_provisional_cannot_transition_to_self(self) -> None:
        """PROVISIONAL cannot transition to itself."""
        status = EnumPatternLifecycleStatus.PROVISIONAL
        assert status.can_transition_to(EnumPatternLifecycleStatus.PROVISIONAL) is False

    def test_validated_cannot_transition_to_self(self) -> None:
        """VALIDATED cannot transition to itself."""
        status = EnumPatternLifecycleStatus.VALIDATED
        assert status.can_transition_to(EnumPatternLifecycleStatus.VALIDATED) is False

    def test_deprecated_cannot_transition_to_self(self) -> None:
        """DEPRECATED cannot transition to itself (terminal state)."""
        status = EnumPatternLifecycleStatus.DEPRECATED
        assert status.can_transition_to(EnumPatternLifecycleStatus.DEPRECATED) is False

    # -------------------------------------------------------------------------
    # Terminal state comprehensive test
    # -------------------------------------------------------------------------

    def test_deprecated_is_terminal_state(self) -> None:
        """DEPRECATED has no valid transitions (terminal state)."""
        status = EnumPatternLifecycleStatus.DEPRECATED
        for target in EnumPatternLifecycleStatus:
            assert status.can_transition_to(target) is False, (
                f"DEPRECATED should not be able to transition to {target.name}"
            )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (EnumPatternLifecycleStatus.CANDIDATE, EnumPatternLifecycleStatus.PROVISIONAL, True),
            (EnumPatternLifecycleStatus.CANDIDATE, EnumPatternLifecycleStatus.DEPRECATED, True),
            (EnumPatternLifecycleStatus.PROVISIONAL, EnumPatternLifecycleStatus.VALIDATED, True),
            (EnumPatternLifecycleStatus.PROVISIONAL, EnumPatternLifecycleStatus.DEPRECATED, True),
            (EnumPatternLifecycleStatus.VALIDATED, EnumPatternLifecycleStatus.DEPRECATED, True),
            # Invalid: skipping stages
            (EnumPatternLifecycleStatus.CANDIDATE, EnumPatternLifecycleStatus.VALIDATED, False),
            # Invalid: backward transitions
            (EnumPatternLifecycleStatus.PROVISIONAL, EnumPatternLifecycleStatus.CANDIDATE, False),
            (EnumPatternLifecycleStatus.VALIDATED, EnumPatternLifecycleStatus.CANDIDATE, False),
            (EnumPatternLifecycleStatus.VALIDATED, EnumPatternLifecycleStatus.PROVISIONAL, False),
            # Invalid: from terminal state
            (EnumPatternLifecycleStatus.DEPRECATED, EnumPatternLifecycleStatus.CANDIDATE, False),
            (EnumPatternLifecycleStatus.DEPRECATED, EnumPatternLifecycleStatus.PROVISIONAL, False),
            (EnumPatternLifecycleStatus.DEPRECATED, EnumPatternLifecycleStatus.VALIDATED, False),
            # Invalid: self-transitions
            (EnumPatternLifecycleStatus.CANDIDATE, EnumPatternLifecycleStatus.CANDIDATE, False),
            (EnumPatternLifecycleStatus.PROVISIONAL, EnumPatternLifecycleStatus.PROVISIONAL, False),
            (EnumPatternLifecycleStatus.VALIDATED, EnumPatternLifecycleStatus.VALIDATED, False),
            (EnumPatternLifecycleStatus.DEPRECATED, EnumPatternLifecycleStatus.DEPRECATED, False),
        ],
    )
    def test_transition_matrix(
        self,
        source: EnumPatternLifecycleStatus,
        target: EnumPatternLifecycleStatus,
        expected: bool,
    ) -> None:
        """Parametrized test covering the complete transition matrix."""
        result = source.can_transition_to(target)
        assert result is expected, (
            f"Transition {source.name} → {target.name} should be "
