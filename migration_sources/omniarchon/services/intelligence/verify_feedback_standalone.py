#!/usr/bin/env python3
"""
Standalone verification script for Pattern Feedback Loop.
Runs without Docker dependencies by importing the module directly.
"""
...
# Load the module directly without triggering parent imports
feedback_module_path = (
    Path(__file__).parent / "src/services/pattern_learning/pattern_feedback.py"
)
spec = importlib.util.spec_from_file_location("pattern_feedback", feedback_module_path)
pattern_feedback = importlib.util.module_from_spec(spec)
spec.loader.exec_module(pattern_feedback)

PatternFeedbackService = pattern_feedback.PatternFeedbackService
ValidationOutcome = pattern_feedback.ValidationOutcome
PatternFeedback = pattern_feedback.PatternFeedback


async def main():
    """Demonstrate feedback loop functionality."""
    print("=" * 70)
    print("Pattern Feedback Loop Verification (Standalone)")
    print("=" * 70)
