...
class RetryMetrics:
    ...
    async def record_retry_reason(self, reason: str):
        """Record a retry reason without incrementing attempt counters."""
        async with self._lock:
            if reason:
                self.retries_by_reason[reason] = (
                    self.retries_by_reason.get(reason, 0) + 1
...
async def retry_async(
    ...
    **kwargs,
    ...
    last_exception: Optional[Exception] = None
    retry_count = 0
    retry_reasons: List[str] = []  # Track reasons for each retry
    ...
    for attempt in range(max_attempts):
        try:
                ...
                if should_retry_status_code(result.status_code):
                    if attempt < max_attempts - 1:
                        ...
                        retry_reasons.append(
                            f"status_{result.status_code}"
                        )  # Track this retry reason

                        # Get delay for this attempt
                        delay = (
                            backoff_delays[attempt]
                            if attempt < len(backoff_delays) and len(backoff_delays) > 0
                            else (
                                backoff_delays[-1] if len(backoff_delays) > 0 else 0.0
                            )
                        )
            ...
            # Record success AFTER retry decision confirmed
            if attempt > 0:
                logger_instance.info(
                    f"{operation_name} succeeded after {attempt} retries "
                    f"(duration: {duration_ms:.2f}ms)"
                )
                # Record single successful attempt with retry count
                await _global_metrics.record_attempt(
                    success=True, retry_count=retry_count, reason=""
                )
                # Record each retry reason separately
                for reason in retry_reasons:
                    await _global_metrics.record_retry_reason(reason)
            else:
                await _global_metrics.record_attempt(
            ...
            last_exception = e
            retry_count += 1
            retry_reasons.append(type(e).__name__)  # Track this retry reason
            ...
            if attempt < max_attempts - 1:
                ...
                delay = (
                    backoff_delays[attempt]
                    if attempt < len(backoff_delays) and len(backoff_delays) > 0
                    else (backoff_delays[-1] if len(backoff_delays) > 0 else 0.0)
                )
            ...
            if should_retry_status_code(e.response.status_code):
                ...
                retry_count += 1
                retry_reasons.append(
                    f"status_{e.response.status_code}"
                )  # Track this retry reason
