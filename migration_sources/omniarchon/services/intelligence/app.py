...
RETRY_BACKOFF_MULTIPLIER = 2.0  # Exponential backoff multiplier (e.g., 1s → 2s → 4s)
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        if CODE_RELATIONSHIP_DETECTOR_AVAILABLE:
            ...
            logger.info(
                "✅ CodeRelationshipDetector initialized successfully - file import extraction ENABLED"
            )
        else:
            logger.error(
                "❌ CodeRelationshipDetector NOT available - this is REQUIRED for file import extraction"
            )
            logger.error(
                "❌ File tree/graph implementation will NOT work without this component"
            )
            code_relationship_detector = None
        ...
        for route in app.routes:
            if hasattr(route, "path") and hasattr(route, "methods"):
                ...
                route_count += 1
        logger.info(f"📋 [STARTUP] Total routes registered: {route_count}")
...
async def _store_entities_background(
    ...
    if memgraph_adapter:
        ...
        logger.info(
            f"✅ Stored {len(entities)} entities from {source_path}",
            extra={
        ...
        logger.warning(
            "⚠️ Memgraph adapter not available - skipping entity storage",
    ...
    if trigger_freshness and freshness_event_coordinator:
        ...
        logger.info(
            f"✅ Triggered freshness analysis for updated document: {source_path}",
...
async def _trigger_freshness_analysis_background(source_path: str):
    ...
    - Backoff multiplier: 2x (0.5s → 1s → 2s)
    - Tracks metrics via Prometheus
    ...
    if not freshness_event_coordinator:
        logger.warning(
            "⚠️ Freshness event coordinator not available - skipping analysis",
            extra={"source_path": source_path},
        )
        return
    ...
    await freshness_event_coordinator.handle_document_update_event(event)
    logger.info(
        f"✅ Triggered standalone freshness analysis for document: {source_path}",
        extra={"source_path": source_path, "operation": "freshness_trigger"},
    )
...
def _enhance_document_metadata(
    ...
    if project_name is not None:
        enhanced["project_name"] = project_name
...
async def process_document_for_indexing(
    ...
    try:
        ...
        # FIX: Ensure project_name is in metadata BEFORE background task
        # This ensures it's included in the logged metadata_fields and passed to background task
        if project_id and "project_name" not in enhanced_metadata:
            enhanced_metadata["project_name"] = project_id

        logger.info(
            f"🔬 [INDEXING PIPELINE] Processing document for indexing | document_id={document_id} | "
            f"project_id={project_id} | content_length={len(full_text)} | source_path={source_path}"
        )

        # Extract entities and relationships from document content
        logger.info(
            f"🔍 [ENRICHMENT STEP] Extracting entities | document_id={document_id} | "
        ...
        logger.info(
            f"✅ [ENRICHMENT STEP] Entities and relationships extracted | document_id={document_id} | "
        ...
        logger.info(
            f"📦 [STORAGE PREP] Preparing data for storage | document_id={document_id} | "
        ...
        logger.info(
            f"✅ [PIPELINE] Document processing queued for background storage | "
...
async def get_document_processing_status(document_id: str):
    ...
    if not tracker:
        raise HTTPException(
            status_code=503, detail="Background task tracking not available"
        )
    ...
    if not status:
        raise HTTPException(
            status_code=404, detail=f"No status found for document_id: {document_id}"
        )
    ...
    for rel in import_relationships:
        try:
            ...
            if success:
                stored_count += 1
                logger.debug(
                    f"✅ [IMPORT STORAGE] Created import relationship | "
            ...
            logger.warning(
                f"⚠️ [IMPORT STORAGE] Failed to store import relationship | "
...
async def _process_document_background(
    ...
    if tracker:
        await tracker.start_task(
            document_id, correlation_id=None
        )  # TODO: Add correlation ID propagation
    ...
    try:
        ...
        if memgraph_adapter:
            try:
                # Ensure project_name is in metadata for File node creation
                # FIX: Explicitly set project_name from project_id parameter (matches DocumentIndexingHandler fix)
                if project_id and "project_name" not in metadata:
                    metadata["project_name"] = project_id

                file_data = _create_file_node(
                    source_path, full_text, metadata, entities
                )
                logger.info(
                    f"📁 [INDEXING PIPELINE] Creating file node | document_id={document_id} | "
                    f"file_path={source_path} | language={file_data['language']} | "
                    f"line_count={file_data['line_count']} | entity_count={file_data['entity_count']} | "
                    f"project_name={file_data.get('project_name', 'unknown')}",
                    extra={
                        "document_id": document_id,
                        "project_id": project_id,
                ...
                if success:
                    logger.info(
                        f"✅ [INDEXING PIPELINE] File node created | document_id={document_id} | "
                    ...
                    logger.warning(
                        f"⚠️ [INDEXING PIPELINE] File node creation returned false | document_id={document_id} | "
                ...
                logger.error(
                    f"❌ [INDEXING PIPELINE] File node creation failed | document_id={document_id} | "
                    f"error={str(e)}",
                    extra={
                        "document_id": document_id,
                        "pipeline_step": "file_node_creation",
        ...
        if memgraph_adapter and entities:
            logger.info(
                f"🔗 [INDEXING PIPELINE] Storing entities in Memgraph | document_id={document_id} | entities_count={len(entities)}",
                extra={
                    "document_id": document_id,
                    "project_id": project_id,
                    "entities_count": len(entities),
            ...
            await memgraph_adapter.store_entities(entities)
            logger.info(
                f"✅ [INDEXING PIPELINE] Entities stored in Memgraph | document_id={document_id}",
                extra={
                    "document_id": document_id,
                    "pipeline_step": "memgraph_storage",
                    "status": "success",
            ...
            if relationships:
                logger.info(
                    f"🔗 [INDEXING PIPELINE] Storing relationships in Memgraph | document_id={document_id} | relationships_count={len(relationships)}",
                    extra={
                        "document_id": document_id,
                        "project_id": project_id,
                ...
                logger.info(
                    f"✅ [INDEXING PIPELINE] Relationships stored in Memgraph | document_id={document_id} | stored_count={stored_count}",
                    extra={
                        "document_id": document_id,
                        "pipeline_step": "memgraph_relationship_storage",
                        "status": "success",
            ...
            logger.warning(
                f"⚠️ [INDEXING PIPELINE] Skipping Memgraph storage | document_id={document_id} | memgraph_available={memgraph_adapter is not None} | entities_count={len(entities)}",
                extra={
                    "document_id": document_id,
                    "pipeline_step": "memgraph_storage",
                    "status": "skipped",
        ...
        if code_relationship_detector and memgraph_adapter:
            ...
            if language in supported_languages:
                try:
                    logger.info(
                        f"📦 [IMPORT EXTRACTION] Extracting file imports | document_id={document_id} | language={language}",
                        extra={
                            "document_id": document_id,
                            "project_id": project_id,
                            "language": language,
                    ...
                    if import_relationships:
                        ...
                        logger.info(
                            f"✅ [IMPORT EXTRACTION] Extracted and stored imports | document_id={document_id} | "
                        ...
                        logger.debug(
                            f"ℹ️ [IMPORT EXTRACTION] No imports found | document_id={document_id}",
                            extra={
                                "document_id": document_id,
                                "pipeline_step": "import_extraction",
                            },
                    ...
                    logger.warning(
                        f"⚠️ [IMPORT EXTRACTION] Failed to extract imports | document_id={document_id} | error={str(e)}",
                        extra={
                            "document_id": document_id,
                            "pipeline_step": "import_extraction",
                            "status": "warning",
                ...
                logger.debug(
                    f"ℹ️ [IMPORT EXTRACTION] Skipping unsupported language | document_id={document_id} | language={language}",
                    extra={
                        "document_id": document_id,
                        "language": language,
                        "pipeline_step": "import_extraction",
        ...
        try:
            ...
            logger.info(
                f"🔍 [VECTORIZATION] Generating embedding | document_id={document_id} | "
            ...
            logger.info(
                f"✅ [VECTORIZATION] Embedding generated | document_id={document_id} | "
            ...
            logger.info(
                f"📊 [VECTORIZATION] Indexing in Qdrant | document_id={document_id} | "
            ...
            if qdrant_result.success and qdrant_result.items_processed > 0:
                vector_indexed = True
                logger.info(
                    f"✅ [VECTORIZATION] Qdrant indexing complete | document_id={document_id} | "
                ...
                logger.warning(
                    f"⚠️ [VECTORIZATION] Qdrant indexing returned no items | document_id={document_id} | "
            ...
            logger.error(
                f"❌ [VECTORIZATION] Vectorization failed | document_id={document_id} | "
        ...
        if freshness_event_coordinator:
            ...
            if source_path.startswith("archon://"):
                logger.info(
                    f"⏭️ [PIPELINE] Skipping freshness analysis for archon:// URI | document_id={document_id} | "
                    f"source_path={source_path}",
                    extra={
                        "document_id": document_id,
                ...
                await freshness_event_coordinator.handle_document_update_event(event)
                logger.info(
                    f"✅ [PIPELINE] Triggered freshness analysis | document_id={document_id}",
                    extra={
                        "document_id": document_id,
                        "pipeline_step": "freshness_analysis",
                        "status": "triggered",
        ...
        logger.info(
            f"✅ [INDEXING PIPELINE] Document processing complete | document_id={document_id} | "
            f"embeddings_generated={embeddings_generated} | vector_indexed={vector_indexed}",
            extra={
                "document_id": document_id,
                "project_id": project_id,
...
async def batch_index_documents(
    ...
    try:
        ...
        logger.info(
            f"🔬 [BATCH INDEXING] Starting batch indexing | total_documents={total_docs} | "
        ...
        for batch_idx in range(0, total_docs, request.batch_size):
            ...
            logger.info(
                f"📦 [BATCH INDEXING] Processing batch {batch_num}/{total_batches} | "
            ...
            for doc_idx, doc in enumerate(batch):
                try:
                    ...
                    logger.info(
                        f"📄 [BATCH INDEXING] Processing document {batch_idx + doc_idx + 1}/{total_docs} | "
                    ...
                    logger.info(
                        f"✅ [BATCH INDEXING] Entities and relationships extracted | document_id={document_id} | "
                    ...
                    logger.error(
                        f"❌ [BATCH INDEXING] Failed to process document | "
        ...
        logger.info(
            f"✅ [BATCH INDEXING] Batch indexing complete | "
        ...
        logger.error(f"❌ [BATCH INDEXING] Batch indexing failed catastrophically: {e}")
        processing_time = time.time() - start_time
        return BatchIndexResult(
            success=False,
...
async def index_directory_hierarchy(
    ...
    try:
        ...
        logger.info(
            f"🌳 [API] Directory hierarchy indexing requested | "
        ...
        logger.info(
            f"✅ [API] Directory hierarchy indexed successfully | "
            f"project={project_name} | "
            f"stats={stats}"
        )
        ...
        logger.error(
            f"❌ [API] Directory hierarchy indexing failed | "
            f"project={project_name} | "
            f"error={str(e)}",
            exc_info=True,
        )
...
async def visualize_tree(
    ...
    Uses the PROJECT → DIRECTORY → FILE graph structure built by DirectoryIndexer.
    ...
    try:
        ...
        logger.info(
            f"🌳 [TREE VIZ] Tree visualization requested | "
            f"project={project_name} | "
        ...
        logger.info(
            f"✅ [TREE VIZ] Tree visualization generated | "
            f"project={project_name} | "
        ...
        logger.error(
            f"❌ [TREE VIZ] Tree visualization failed | "
            f"project={project_name} | "
            f"error={str(e)}",
            exc_info=True,
        )
...
async def detect_orphans(project_name: str, entry_points: Optional[str] = None):
    ...
    try:
        ...
        logger.info(
            f"🔍 [ORPHAN API] Orphan detection requested | "
            f"project={project_name} | "
        ...
        logger.info(
            f"✅ [ORPHAN API] Orphan detection complete | "
            f"project={project_name} | "
        ...
        logger.error(
            f"❌ [ORPHAN API] Orphan detection failed | "
            f"project={project_name} | "
            f"error={str(e)}",
            exc_info=True,
        )
...
async def get_orphan_summary(project_name: str):
    ...
    try:
        ...
        logger.info(
            f"🔍 [ORPHAN API] Orphan summary requested | " f"project={project_name}"
        )
        ...
        logger.info(
            f"✅ [ORPHAN API] Orphan summary complete | "
            f"project={project_name} | "
        ...
        logger.error(
            f"❌ [ORPHAN API] Orphan summary failed | "
            f"project={project_name} | "
            f"error={str(e)}",
            exc_info=True,
        )
...
async def get_dead_code_files(project_name: str):
    ...
    try:
        ...
        logger.info(
            f"🔍 [ORPHAN API] Dead code files requested | " f"project={project_name}"
        )
        ...
        logger.info(
            f"✅ [ORPHAN API] Dead code files retrieved | "
            f"project={project_name} | "
        ...
        logger.error(
            f"❌ [ORPHAN API] Dead code retrieval failed | "
            f"project={project_name} | "
            f"error={str(e)}",
            exc_info=True,
        )
...
async def _build_tree_structure(
    ...
    Queries PROJECT → DIRECTORY → FILE hierarchy using variable-length
    ...
    try:
        ...
        logger.error(
            f"❌ [TREE BUILD] Failed to build tree structure | "
            f"project={project_name} | "
            f"error={str(e)}",
            exc_info=True,
        )
...
async def _build_node_recursively(
    ...
    try:
        ...
        logger.error(
            f"❌ [TREE BUILD] Failed to build node recursively | "
...
async def _extract_dependencies(project_name: str) -> List[DependencyEdge]:
    ...
    try:
        ...
        logger.debug(
            f"🌳 [DEPENDENCIES] Extracted {len(dependencies)} dependencies | "
            f"project={project_name}"
        )
        ...
        logger.error(
            f"❌ [DEPENDENCIES] Failed to extract dependencies | "
            f"project={project_name} | "
            f"error={str(e)}",
            exc_info=True,
        )
...
async def _gather_tree_statistics(
    ...
    try:
        ...
        logger.debug(
            f"🌳 [STATISTICS] Gathered tree statistics | "
            f"project={project_name} | "
        ...
        logger.error(
            f"❌ [STATISTICS] Failed to gather statistics | "
            f"project={project_name} | "
            f"error={str(e)}",
            exc_info=True,
        )
