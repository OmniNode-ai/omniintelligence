"""
Integration Test: Custom Quality Rules - Enable
Tests complete flow: Event → Handler → HTTP → Response

Wave 6 - HTTP Implementation
Operation: PUT /api/custom-rules/project/{project_id}/rule/{rule_id}/enable
"""
...
class TestCustomRulesEnableIntegration:
    """Integration tests for Custom Rules Enable operation."""

    @pytest.mark.asyncio
    async def test_enable_success_flow(
        self,
        mock_event_envelope,
        mock_router,
    ):
        """Test complete enable flow with successful HTTP response."""
        ...
        event = mock_event_envelope(
            correlation_id, payload, event_type="ENABLE_REQUESTED"
        )
        ...
        with patch("httpx.AsyncClient") as mock_client_class:
            ...
            mock_client.put.assert_called_once_with(
                "http://localhost:8053/api/custom-rules/project/test_project_001/rule/naming_convention/enable"
            )
            ...
            assert result_payload["enabled"] is True
