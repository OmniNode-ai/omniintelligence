...
from agents.base_agent import ArchonDependencies, BaseAgent

# REMOVED: MCP client dependency - agents need refactoring to use direct service calls
...
class DocumentDependencies(ArchonDependencies):
    ...
    project_id: str = (
        ""  # Required but needs default value due to parent class having defaults
    )
...
class DocumentOperation(BaseModel):
    ...
    operation_type: str = Field(
        description="Type of operation: create, update, delete, query"
    )
...
class DocumentAgent(BaseAgent[DocumentDependencies, DocumentOperation]):
    ...
    def _create_agent(self, **kwargs) -> Agent:
        ...
        agent = Agent(
            ...
            system_prompt="""You are a Document Management Assistant that helps users create, update, \
and modify project documents through conversation.
...
"""
        ...
        async def get_document(
            ctx: RunContext[DocumentDependencies], document_title: str
        ) -> str:
            ...
            try:
                ...
                matching_docs = [
                    doc
                    for doc in docs
                    if document_title.lower() in doc.get("title", "").lower()
                ]
        ...
        async def create_document(
            ...
            try:
                ...
                blocks = self._convert_to_blocks(
                    title, document_type, content_description
                )
                ...
                return f"Error creating document: {e!s}"
        ...
        async def update_document(
            ...
            try:
                # TODO: Refactor to use direct service calls instead of MCP
                raise NotImplementedError(
                    "Document management requires refactoring after MCP removal. "
                    "Use direct DocumentService or API calls to archon-server"
                )
                ...
                if section_to_update in current_content:
                    if isinstance(current_content[section_to_update], list):
                        ...
                        if new_content.startswith("[") and new_content.endswith("]"):
                            try:
                                current_content[section_to_update] = json.loads(
                                    new_content
                                )
                            except (json.JSONDecodeError, ValueError) as e:
                                logger.warning(
                                    f"Failed to parse new_content as JSON list: {e}"
                                )
                        ...
                        try:
                            ...
                            logger.warning(
                                f"Failed to parse new_content as JSON dict: {e}"
                            )
                    ...
                    try:
                        ...
                        logger.warning(f"Failed to parse new_content as JSON: {e}")
                        current_content[section_to_update] = new_content

                # TODO: Update document via direct service call
                # (Already unreachable due to NotImplementedError above)
                pass
                ...
                return f"Error updating document: {e!s}"
            ...
            try:
                ...
                content = {
                    ...
                    "technical_notes": {
                        ...
                        "backend_endpoints": [
                            f"/api/{feature_name.lower().replace(' ', '-')}"
                        ],
                ...
                # TODO: Refactor to use direct service calls instead of MCP
                raise NotImplementedError(
                    "Feature plan creation requires refactoring after MCP removal. "
                    "Use direct ProjectService or API calls"
                )
                ...
                if result_data.get("success", False):
                    return (
                        f"Successfully created React Flow feature plan for '{feature_name}'. "
                        "The plan includes a visual flow with 5 nodes and user story breakdown. "
                        "You can now view and edit this in the project documents."
                    )
                else:
                ...
                return f"Error creating feature plan: {e!s}"
        ...
        async def create_erd(
            ...
            try:
                ...
                for line in entity_lines:
                    ...
                    if line and not line.startswith("-"):
                        ...
                        if "id" in attr_name.lower():
                            ...
                            "created" in attr_name.lower()
                            or "updated" in attr_name.lower()
                        ):
                            attr_type = "TIMESTAMP"
                        elif (
                            ...
                            "price" in attr_name.lower() or "cost" in attr_name.lower()
                        ):
                            attr_type = "DECIMAL(10,2)"
                        elif (
                            "active" in attr_name.lower()
                            or "enabled" in attr_name.lower()
                        ):
                            attr_type = "BOOLEAN"
                ...
                for entity in entities:
                    table_sql = (
                        f"CREATE TABLE {entity['name'].lower().replace(' ', '_')} (\n"
                    )
                ...
                # TODO: Refactor to use direct service calls instead of MCP
                raise NotImplementedError(
                    "ERD creation requires refactoring after MCP removal. "
                    "Use direct ProjectService or API calls"
                )
                ...
                if result_data.get("success", False):
                    return (
                        f"Successfully created ERD for '{system_name}' with {len(entities)} entities. "
                        "Generated SQL schema and relationship mappings. "
                        "The ERD includes detailed entity definitions and can be imported into database design tools."
                    )
                else:
                ...
                return f"Error creating ERD: {e!s}"
        ...
        async def request_approval(
            ...
            try:
                ...
                # TODO: Refactor to use direct service calls instead of MCP
                raise NotImplementedError(
                    "Approval request creation requires refactoring after MCP removal. "
                    "Use direct DocumentService or API calls"
                )
                ...
                if result_data.get("success", False):
                    return (
                        f"Approval request created for changes to '{document_title}'. "
                        "Status: Pending approval from Product Manager and Technical Lead. "
                        f"Deadline: 3 days. Change summary: {change_summary}"
                    )
                else:
                ...
                return f"Error creating approval request: {e!s}"
    ...
    def _convert_to_blocks(
        ...
        if document_type == "prd":
            ...
            blocks.append(
                self._create_block(
                    "bulleted_list", "Identify key stakeholders and their needs"
                )
            )
            ...
            blocks.append(
                self._create_block(
                    "paragraph",
                    "**In Scope:** Core features and functionality to be delivered",
                )
            )
            blocks.append(
                self._create_block(
            ...
            blocks.append(
                self._create_block(
                    "bulleted_list", "Frontend: React, TypeScript, Tailwind CSS"
                )
            )
            blocks.append(
                self._create_block("bulleted_list", "Backend: FastAPI, Python")
            )
            blocks.append(
                self._create_block("bulleted_list", "Database: Supabase (PostgreSQL)")
            )
            blocks.append(
                self._create_block(
                    "bulleted_list", "Infrastructure: Docker, Cloud deployment"
                )
            )
            ...
            blocks.append(
                self._create_block(
                    "paragraph", "Key user stories and acceptance criteria"
                )
            )
            ...
            blocks.append(
                self._create_block("paragraph", "Project phases and delivery timeline")
            )
            ...
            blocks.append(
                self._create_block(
                    "paragraph", "Identified risks and mitigation strategies"
                )
            )
            ...
            blocks.append(
                self._create_block(
                    "paragraph", "System architecture and design decisions"
                )
            )

            blocks.append(self._create_block("heading_2", "API Design"))
            blocks.append(
                self._create_block("paragraph", "API endpoints and data models")
            )

            blocks.append(self._create_block("heading_2", "Database Schema"))
            blocks.append(
                self._create_block("paragraph", "Database design and relationships")
            )
            ...
            blocks.append(
                self._create_block("paragraph", f"Topic: {content_description}")
            )

            blocks.append(self._create_block("heading_2", "Attendees"))
            blocks.append(
                self._create_block("paragraph", "List of meeting participants")
            )
    ...
    def get_system_prompt(self) -> str:
        ...
        try:
            ...
            return (
                "Document Management Assistant for conversational document operations."
            )
    ...
    async def run_conversation(
        ...
        try:
            ...
            self.logger.error(f"Document operation failed: {e!s}")
            # Return error result
