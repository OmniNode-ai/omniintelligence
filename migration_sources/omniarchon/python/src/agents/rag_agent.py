...
"""
        ...
        async def search_documents(
            ...
            try:
                ...
                # MCP client removed - need to call intelligence/search service directly
                raise NotImplementedError(
                    "RAG search requires refactoring after MCP removal. "
                    "Use direct calls to archon-search service at localhost:8055"
                )
        ...
        async def list_available_sources(ctx: RunContext[RagDependencies]) -> str:
            ...
            try:
                # TODO: Refactor to use direct service calls instead of MCP
                raise NotImplementedError(
                    "Get available sources requires refactoring after MCP removal. "
                    "Use direct calls to archon-search service at localhost:8055"
                )
                ...
                return f"Error retrieving sources: {e!s}"
        ...
        async def search_code_examples(
            ...
            try:
                ...
                # TODO: Refactor to use direct service calls instead of MCP
                raise NotImplementedError(
                    "Search code examples requires refactoring after MCP removal. "
                    "Use direct calls to archon-search service at localhost:8055"
                )
        ...
        async def refine_search_query(
            ...
            try:
                ...
                return f"Could not refine query: {e!s}"
    ...
    async def run_conversation(
        ...
        try:
            ...
            self.logger.error(f"RAG query failed: {e!s}")
            # Return error result
            return RagQueryResult(
                query_type="error",
                original_query=user_message,
                ...
                results_found=0,
                sources=[],
                answer=f"I encountered an error while searching: {e!s}",
                citations=[],
                success=False,
                message=f"Failed to process query: {e!s}",
            )
