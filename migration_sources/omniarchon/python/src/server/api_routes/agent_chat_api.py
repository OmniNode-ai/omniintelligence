...
async def create_session(request: CreateSessionRequest):
    ...
    logger.info(
        f"Created chat session {session_id} with agent_type: {request.agent_type}"
    )
...
async def send_message(session_id: str, request: dict):
    ...
    await sio.emit(
        "message", {"type": "message", "data": user_msg}, room=f"chat_{session_id}"
    )

    # Trigger agent response via Socket.IO
    asyncio.create_task(
        process_agent_response(
            session_id, request.get("message", ""), request.get("context", {})
        )
    )
...
async def chat_message(sid, data):
    ...
    if not session_id or not message:
        await sio.emit(
            "error", {"type": "error", "error": "Missing session_id or message"}, to=sid
        )
        return
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=httpx.Timeout(60.0)) as client:
            async with client.stream(
                ...
                await sio.emit(
                    "message", {"type": "message", "data": agent_msg}, room=room
                )

                # Emit stream complete
                await sio.emit(
                    "stream_complete", {"type": "stream_complete"}, room=room
                )
