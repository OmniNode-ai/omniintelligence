        ...
        if execution_id in self.connections:
            ...
            for websocket in self.connections[execution_id]:
                try:
                    ...
                    logger.debug(
                        f"Failed to send message to WebSocket (execution {execution_id}): {e}"
                    )
...
async def execute_mcp_tests(execution_id: str) -> TestExecution:
    ...
    try:
        ...
        logger.info(
            f"[DEBUG] /app/tests directory exists: {os.path.exists('/app/tests')}"
        )
        test_files = [
        ...
        test_files_exist = [os.path.exists(f"/app/{f}") for f in test_files]
        logger.info(f"[DEBUG] Test files exist: {test_files_exist}")
        ...
        logger.info(
            f"[DEBUG] Process created with PID: {process.pid if process else 'None'}"
        )
        ...
        if exit_code == 0:
            ...
            execution.summary = {
                "result": "All Python tests passed",
                "exit_code": exit_code,
            }
        else:
            execution.status = TestStatus.FAILED
            execution.summary = {
                "result": "Some Python tests failed",
                "exit_code": exit_code,
            }
...
async def execute_ui_tests(execution_id: str) -> TestExecution:
    ...
    try:
        ...
        execution.summary = {
            "result": "All React UI tests passed (simulated)",
            "exit_code": 0,
        }
...
async def stream_process_output(execution_id: str, process: asyncio.subprocess.Process):
    ...
    execution = test_executions[execution_id]
    logger.info(
        f"[DEBUG] Starting stream_process_output for execution_id: {execution_id}"
    )
    ...
    while True:
        try:
            ...
            logger.error(f"[DEBUG] Exception details: {e!s}")
            break
...
async def run_mcp_tests(
    request: TestExecutionRequest, background_tasks: BackgroundTasks
):
    ...
    logfire.info(
        f"Starting MCP test execution | execution_id={execution_id} | test_type=mcp"
    )
    ...
    logfire.info(
        f"MCP test execution queued successfully | execution_id={execution_id}"
    )
...
async def run_ui_tests(
    ...
    logger.info(f"[DEBUG] Request: {request}")
    logfire.info(
        f"Starting UI test execution | execution_id={execution_id} | test_type=ui"
    )
...
async def get_test_status(execution_id: str):
    ...
    try:
        ...
        logfire.error(
            f"Failed to get test status | error={e!s} | execution_id={execution_id}"
        )
...
async def get_test_history(limit: int = 50, offset: int = 0):
    ...
    try:
        logfire.info(
            f"Getting test execution history | limit={limit} | offset={offset}"
        )
        ...
        return TestHistoryResponse(
            executions=execution_responses, total_count=total_count
        )

    except Exception as e:
        logfire.error(
            f"Failed to get test history | error={e!s} | limit={limit} | offset={offset}"
        )
        raise HTTPException(status_code=500, detail=str(e)) from e
...
async def cancel_test_execution(execution_id: str):
    ...
    try:
        ...
        if execution.status not in [TestStatus.PENDING, TestStatus.RUNNING]:
            ...
            raise HTTPException(
                status_code=400, detail="Test execution cannot be cancelled"
            )
        ...
        if execution.process:
            try:
                ...
                logfire.warning(
                    f"Error terminating test process | error={e!s} | execution_id={execution_id}"
                )
        ...
        logfire.info(
            f"Test execution cancelled successfully | execution_id={execution_id}"
        )
        ...
        logfire.error(
            f"Failed to cancel test execution | error={e!s} | execution_id={execution_id}"
        )
        raise HTTPException(status_code=500, detail=str(e)) from e
...
async def get_latest_test_results():
    ...
    try:
        ...
        if not latest_execution:
            raise HTTPException(
                status_code=404, detail="No completed test results available"
            )
