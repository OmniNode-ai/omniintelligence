...
async def broadcast_project_update():
    ...
    try:
        ...
        formatted_projects = source_service.format_projects_with_sources(
            result["projects"]
        )

        await sio.emit(
            "projects_update", {"projects": formatted_projects}, room="project_list"
        )
        logger.info(
            f"Broadcasted project list update with {len(formatted_projects)} projects"
        )
...
async def broadcast_crawl_progress(progress_id: str, data: dict):
    ...
    if (
        time_since_last < _min_broadcast_interval
        and current_status not in important_statuses
    ):
    ...
    try:
        ...
        logger.debug(
            f"Room {progress_id} has {len(room_sids)} subscribers: {room_sids}"
        )
        logger.debug(
            f"All rooms in namespace '/': {list(all_rooms.get('/', {}).keys())}"
        )
...
async def connect(sid, environ):
    ...
    logger.info(
        f"🔌 [SOCKETIO] User-Agent: {headers.get('HTTP_USER_AGENT', 'unknown')}"
    )
    ...
    try:
        ...
        # Failed to get room statistics - non-critical, connection still succeeds
        logger.debug(f"Could not retrieve connected client count: {e}")
...
async def subscribe_projects(sid, data=None):
    ...
    try:
        ...
        if not success:
            await sio.emit(
                "error",
                {"message": result.get("error", "Failed to load projects")},
                to=sid,
            )
            return
...
async def subscribe_progress(sid, data):
    ...
    logger.info(
        f"🔔 [SOCKETIO] Received subscribe_progress from {sid} with data: {data}"
    )
    ...
    try:
        ...
        if current_status:
            ...
            if "start_time" in current_status_copy and hasattr(
                ...
                current_status_copy["start_time"] = current_status_copy[
                    "start_time"
                ].isoformat()
            ...
            logger.warning(
                f"📤 [SOCKETIO] No progress operation found for {progress_id}"
            )
    except Exception as e:
...
async def crawl_subscribe(sid, data=None):
    ...
    await sio.enter_room(sid, progress_id)
    logger.info(
        f"✅ [SOCKETIO] Client {sid} subscribed to crawl progress room: {progress_id}"
    )
    ...
    try:
        ...
        logger.debug(
            f"Room '{progress_id}' membership confirmed: {progress_id in client_rooms}"
        )
    ...
    if "error" not in task_status:
        ...
        logger.info(
            f"📤 [SOCKETIO] Sent current crawl state to reconnecting client {sid}"
        )
    else:
    ...
    logger.info(
        f"📤 [SOCKETIO] Sent subscription acknowledgment to {sid} for {progress_id}"
    )
...
async def crawl_unsubscribe(sid, data):
    ...
    if progress_id:
        ...
        logger.debug(
            f"Client {sid} requesting to unsubscribe from crawl progress {progress_id}"
        )
        logger.debug(f"Unsubscribe data: {data}")

        await sio.leave_room(sid, progress_id)
        logger.info(
            f"📤 [SOCKETIO] Client {sid} left crawl progress room: {progress_id}"
        )
...
async def crawl_stop(sid, data):
    ...
    try:
        ...
        if orchestration:
            ...
            logger.warning(
                f"⚠️  [SOCKETIO] No active orchestration found for {progress_id}"
            )
        ...
        logger.error(
            f"❌ [SOCKETIO] Failed to stop crawl | error={e!s} | progress_id={progress_id}"
        )
        await sio.emit(
            "crawl:error",
            {
...
async def join_document_room(sid, data):
    ...
    if not project_id or not document_id:
        await sio.emit(
            "error", {"message": "project_id and document_id required"}, to=sid
        )
        return
...
async def document_change(sid, data):
    ...
    try:
        ...
        await sio.emit("error", {"message": f"Failed to process change: {e!s}"}, to=sid)
...
async def document_batch_update(sid, data):
    ...
    try:
        ...
        if not all([project_id, document_id, changes_data]):
            await sio.emit(
                "error",
                {"message": "project_id, document_id, and changes required"},
                to=sid,
            )
            return
        ...
        if conflicts:
            await sio.emit(
                "conflicts_detected",
                {"conflicts": conflicts, "document_id": document_id},
                to=sid,
            )
        ...
        await sio.emit("error", {"message": f"Failed to process batch: {e!s}"}, to=sid)
...
async def process_document_change(
    ...
    if broadcast:
        ...
        logger.info(
            f"📄 [DOCUMENT SYNC] Broadcasted {change.change_type} change for {document_id}"
        )
...
async def lock_document(sid, data):
    ...
    if document_id in document_locks:
        ...
        if (
            existing_lock["expiry"] > current_time
...
async def delete_document(sid, data):
    ...
    if not all([document_id, project_id, user_id]):
        await sio.emit(
            "error",
            {"message": "document_id, project_id, and user_id required"},
            to=sid,
        )
        return
...
# ============================================================================
# Intelligence WebSocket Event Handlers
# ============================================================================


async def broadcast_intelligence_update(repository: str, intelligence_data: dict):
    """Broadcast new intelligence data to intelligence dashboard subscribers."""
    try:
        # Broadcast to general intelligence room
        await sio.emit(
            "intelligence_update",
            {
            ...
            room="intelligence_dashboard",
        )

        # Also broadcast to repository-specific room
        repository_room = f"intelligence_{repository}"
        await sio.emit(
            "intelligence_update",
            {
                "type": "intelligence_update",
            ...
            room=repository_room,
        )

        logger.info(
            f"🧠 [INTELLIGENCE] Broadcasted intelligence update for {repository}"
        )

    except Exception as e:
        logger.error(f"🧠 [INTELLIGENCE] Error broadcasting intelligence update: {e}")


async def broadcast_correlation_detected(correlation_data: dict):
    """Broadcast when cross-repository correlation is detected."""
    try:
        await sio.emit(
        ...
        logger.info("🧠 [INTELLIGENCE] Broadcasted correlation detection")

    except Exception as e:
        logger.error(f"🧠 [INTELLIGENCE] Error broadcasting correlation: {e}")


async def broadcast_breaking_change_detected(breaking_change_data: dict):
    """Broadcast when breaking changes are detected."""
    try:
        await sio.emit(
            "breaking_change_detected",
            {
                "type": "breaking_change_detected",
                "data": breaking_change_data,
                "timestamp": time.time() * 1000,
                "severity": breaking_change_data.get("severity", "UNKNOWN"),
            },
            room="intelligence_dashboard",
        )

        logger.info("🧠 [INTELLIGENCE] Broadcasted breaking change detection")

    except Exception as e:
        logger.error(f"🧠 [INTELLIGENCE] Error broadcasting breaking change: {e}")


@sio.event
async def join_intelligence_room(sid, data):
    """Join intelligence dashboard room for real-time updates."""
    try:
        room_type = data.get("room_type", "general")  # general or repository-specific
        repository = data.get("repository")

        if room_type == "general":
            # Join general intelligence dashboard room
            await sio.enter_room(sid, "intelligence_dashboard")
            logger.info(f"🧠 [INTELLIGENCE] Client {sid} joined intelligence dashboard")

        elif room_type == "repository" and repository:
            # Join repository-specific intelligence room
            repository_room = f"intelligence_{repository}"
            await sio.enter_room(sid, repository_room)
            logger.info(
                f"🧠 [INTELLIGENCE] Client {sid} joined {repository} intelligence room"
            )

        else:
            await sio.emit(
                "error", {"message": "Invalid room_type or missing repository"}, to=sid
            )
            return

        # Send confirmation
        await sio.emit(
            "intelligence_room_joined",
            {
                "room_type": room_type,
                "repository": repository,
                "timestamp": time.time() * 1000,
        ...
        logger.error(f"🧠 [INTELLIGENCE] Error joining intelligence room: {e}")
        await sio.emit(
            "error", {"message": f"Failed to join intelligence room: {e!s}"}, to=sid
        )


@sio.event
async def leave_intelligence_room(sid, data):
    """Leave intelligence dashboard room."""
    try:
        room_type = data.get("room_type", "general")
        repository = data.get("repository")

        if room_type == "general":
            await sio.leave_room(sid, "intelligence_dashboard")
            logger.info(f"🧠 [INTELLIGENCE] Client {sid} left intelligence dashboard")

        elif room_type == "repository" and repository:
            repository_room = f"intelligence_{repository}"
            await sio.leave_room(sid, repository_room)
            logger.info(
                f"🧠 [INTELLIGENCE] Client {sid} left {repository} intelligence room"
            )

        # Send confirmation
        await sio.emit(
            "intelligence_room_left",
            {
                "room_type": room_type,
                "repository": repository,
                "timestamp": time.time() * 1000,
        ...
        logger.error(f"🧠 [INTELLIGENCE] Error leaving intelligence room: {e}")


@sio.event
async def request_intelligence_stats(sid, data):
    """Request current intelligence statistics."""
    try:
        # Import from shared service to avoid circular imports
        from ..services.intelligence_service import get_intelligence_stats

        repository = data.get("repository")
        time_range = data.get("time_range", "24h")

        # Get current stats using shared service
        stats = await get_intelligence_stats(
            repository=repository, time_range=time_range
        )

        # Send stats to requesting client
        await sio.emit(
            "intelligence_stats_update",
            {
                "type": "intelligence_stats_update",
                "stats": stats.dict(),
                "timestamp": time.time() * 1000,
            },
            to=sid,
        ...
        logger.info(f"🧠 [INTELLIGENCE] Sent intelligence stats to client {sid}")

    except Exception as e:
        logger.error(f"🧠 [INTELLIGENCE] Error getting intelligence stats: {e}")
        await sio.emit(
            "error", {"message": f"Failed to get intelligence stats: {e!s}"}, to=sid
        )


logger.info("🧠 [INTELLIGENCE] Intelligence WebSocket handlers initialized")
