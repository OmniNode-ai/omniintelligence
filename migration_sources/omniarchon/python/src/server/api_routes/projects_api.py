...
from server.config.logfire_config import get_logger, logfire
from server.utils import get_database_client

logger = get_logger(__name__)

# Import Socket.IO broadcast functions from socketio_handlers
from server.api_routes.socketio_handlers import broadcast_project_update
...
async def list_projects(include_content: bool = True):
    ...
    try:
        ...
        logfire.error(f"Failed to list projects | error={e!s}")
...
async def create_project(request: CreateProjectRequest):
    ...
    try:
        ...
        logfire.error(
            f"Failed to start project creation | error={e!s} | title={request.title}"
        )
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def _create_project_with_ai(progress_id: str, request: CreateProjectRequest):
    ...
    try:
        ...
        logfire.error(f"Project creation failed: {e!s}")
...
async def projects_health():
    ...
    try:
        ...
        try:
            ...
            logfire.warning(f"Projects table not found | error={e!s}")
        ...
        try:
            ...
            logfire.warning(f"Tasks table not found | error={e!s}")
        ...
        logfire.error(f"Projects health check failed | error={e!s}")
        return {
            "status": "error",
            "service": "projects",
            "error": str(e),
...
async def get_project(project_id: str):
    ...
    try:
        ...
        logfire.error(f"Failed to get project | error={e!s} | project_id={project_id}")
        raise HTTPException(status_code=500, detail={"error": str(e)})
    ...
    try:
        ...
        if not success:
            if "not found" in result.get("error", "").lower():
                raise HTTPException(
                    status_code=404,
                    detail={"error": f"Project with ID {project_id} not found"},
                )
            else:
                raise HTTPException(status_code=500, detail=result)
        ...
        logfire.error(f"Project update failed | project_id={project_id} | error={e!s}")
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def delete_project(project_id: str):
    ...
    try:
        ...
        logfire.error(
            f"Failed to delete project | error={e!s} | project_id={project_id}"
        )
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def get_project_features(project_id: str):
    ...
    try:
        ...
        logfire.error(
            f"Failed to get project features | error={e!s} | project_id={project_id}"
        )
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def list_project_tasks(
    project_id: str, include_archived: bool = False, exclude_large_fields: bool = False
):
    ...
    try:
        ...
        logfire.error(
            f"Failed to list project tasks | error={e!s} | project_id={project_id}"
        )
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def create_task(request: CreateTaskRequest):
    ...
    try:
        # Debug logging for parent_task_id at API layer
        logfire.info(f"API received parent_task_id: {request.parent_task_id}")
        logfire.info(f"Full CreateTaskRequest: {request}")
        ...
        logfire.error(
            f"Failed to create task | error={e!s} | project_id={request.project_id}"
        )
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def list_tasks(
    ...
    try:
        ...
        logfire.error(f"Failed to list tasks | error={e!s}")
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def get_task(task_id: str):
    ...
    try:
        ...
        logfire.error(f"Failed to get task | error={e!s} | task_id={task_id}")
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def update_task(task_id: str, request: UpdateTaskRequest):
    ...
    try:
        ...
        if request.parent_task_id is not None:
            update_fields["parent_task_id"] = request.parent_task_id
        ...
        logfire.error(f"Failed to update task | error={e!s} | task_id={task_id}")
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def delete_task(task_id: str):
    ...
    try:
        ...
        logfire.error(f"Failed to archive task | error={e!s} | task_id={task_id}")
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def mcp_update_task_status_with_socketio(task_id: str, status: str):
    ...
    try:
        ...
        logfire.error(
            f"Failed to update task status with Socket.IO | error={e!s} | task_id={task_id}"
        )
...
async def list_project_documents(project_id: str, include_content: bool = False):
    ...
    try:
        ...
        logfire.error(
            f"Failed to list documents | error={e!s} | project_id={project_id}"
        )
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def create_project_document(project_id: str, request: CreateDocumentRequest):
    ...
    try:
        ...
        logfire.error(
            f"Failed to create document | error={e!s} | project_id={project_id}"
        )
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def get_project_document(project_id: str, doc_id: str):
    ...
    try:
        ...
        logfire.error(
            f"Failed to get document | error={e!s} | project_id={project_id} | doc_id={doc_id}"
        )
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def update_project_document(
    ...
    try:
        ...
        logfire.error(
            f"Failed to update document | error={e!s} | project_id={project_id} | doc_id={doc_id}"
        )
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def delete_project_document(project_id: str, doc_id: str):
    ...
    try:
        ...
        logfire.error(
            f"Failed to delete document | error={e!s} | project_id={project_id} | doc_id={doc_id}"
        )
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def list_project_versions(project_id: str, field_name: str = None):
    ...
    try:
        ...
        logfire.error(
            f"Failed to list versions | error={e!s} | project_id={project_id}"
        )
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def create_project_version(project_id: str, request: CreateVersionRequest):
    ...
    try:
        ...
        logfire.error(
            f"Failed to create version | error={e!s} | project_id={project_id}"
        )
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def get_project_version(project_id: str, field_name: str, version_number: int):
    ...
    try:
        ...
        logfire.error(
            f"Failed to get version | error={e!s} | project_id={project_id} | field_name={field_name} | version_number={version_number}"
        )
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def restore_project_version(
    ...
    try:
        ...
        logfire.error(
            f"Failed to restore version | error={e!s} | project_id={project_id} | field_name={field_name} | version_number={version_number}"
        )
        raise HTTPException(status_code=500, detail={"error": str(e)})
