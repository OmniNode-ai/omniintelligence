...
# Import logging
from server.config.logfire_config import logfire
...
async def list_credentials(category: str | None = None):
    ...
    try:
        ...
        logfire.error(f"Error listing credentials | category={category} | error={e!s}")
...
async def get_credentials_by_category(category: str):
    ...
    try:
        ...
        logfire.error(
            f"Error getting credentials by category | category={category} | error={e!s}"
        )
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def create_credential(request: CredentialRequest):
    ...
    try:
        ...
        logfire.error(f"Error creating credential | key={request.key} | error={e!s}")
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def get_credential(key: str, decrypt: bool = True):
    ...
    try:
        ...
        logfire.error(f"Error getting credential | key={key} | error={e!s}")
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def update_credential(key: str, request: dict[str, Any]):
    ...
    try:
        ...
        logfire.error(f"Error updating credential | key={key} | error={e!s}")
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def delete_credential(key: str):
    ...
    try:
        ...
        logfire.error(f"Error deleting credential | key={key} | error={e!s}")
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def initialize_credentials_endpoint():
    ...
    try:
        ...
        logfire.error(f"Error reloading credentials | error={e!s}")
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def database_metrics():
    ...
    try:
        ...
        logfire.error(f"Error getting database metrics | error={e!s}")
        raise HTTPException(status_code=500, detail={"error": str(e)})
