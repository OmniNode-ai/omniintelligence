...
from server.services.crawler_manager import get_crawler
from server.services.crawling import CrawlOrchestrationService
from server.services.knowledge import DatabaseMetricsService, KnowledgeItemService
from server.services.search.rag_service import RAGService
from server.services.storage import DocumentStorageService
from server.utils import get_database_client
from server.utils.document_processing import extract_text_from_document

# Get logger for this module
logger = get_logger(__name__)
from server.api_routes.socketio_handlers import (
...
async def get_knowledge_sources():
    ...
    try:
        ...
        safe_logfire_error(f"Failed to get knowledge sources | error={e!s}")
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def get_knowledge_items(
    ...
    try:
        ...
        service = KnowledgeItemService(get_database_client())
        ...
        safe_logfire_error(
            f"Failed to get knowledge items | error={e!s} | page={page} | per_page={per_page}"
        )
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def update_knowledge_item(source_id: str, updates: dict):
    ...
    try:
        ...
        safe_logfire_error(
            f"Failed to update knowledge item | error={e!s} | source_id={source_id}"
        )
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def delete_knowledge_item(source_id: str):
    ...
    try:
        ...
        source_service = SourceManagementService(get_database_client())
        ...
        safe_logfire_error(
            f"Failed to delete knowledge item | error={e!s} | source_id={source_id}"
        )
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def get_knowledge_item_code_examples(source_id: str):
    ...
    try:
        ...
        safe_logfire_error(
            f"Failed to fetch code examples | error={e!s} | source_id={source_id}"
        )
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def refresh_knowledge_item(source_id: str):
    ...
    try:
        ...
        try:
            ...
            safe_logfire_error(f"Failed to get crawler | error={e!s}")
            raise HTTPException(
                status_code=500,
                detail={"error": f"Failed to initialize crawler: {e!s}"},
            )

        # Use the same crawl orchestration as regular crawl
        crawl_service = CrawlOrchestrationService(
            crawler=crawler, supabase_client=get_database_client()
        )
        ...
        safe_logfire_error(
            f"Failed to refresh knowledge item | error={e!s} | source_id={source_id}"
        )
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def crawl_knowledge_item(request: KnowledgeItemRequest):
    ...
    try:
        safe_logfire_info(
            f"Starting knowledge item crawl | url={request.url!s} | knowledge_type={request.knowledge_type} | tags={request.tags}"
        )
        # Generate unique progress ID
        progress_id = str(uuid.uuid4())
        # Start progress tracking with initial state
        ...
        active_crawl_tasks[progress_id] = task
        safe_logfire_info(
            f"Crawl started successfully | progress_id={progress_id} | url={request.url!s}"
        )
        response_data = {
            "success": True,
            "progressId": progress_id,
        ...
        safe_logfire_error(f"Failed to start crawl | error={e!s} | url={request.url!s}")
...
async def _perform_crawl_with_progress(progress_id: str, request: KnowledgeItemRequest):
    ...
    async with crawl_semaphore:
        safe_logfire_info(
            f"Acquired crawl semaphore | progress_id={progress_id} | url={request.url!s}"
        )
        try:
            safe_logfire_info(
                f"Starting crawl with progress tracking | progress_id={progress_id} | url={request.url!s}"
            )
            ...
            try:
                ...
                await error_crawl_progress(
                    progress_id, f"Failed to initialize crawler: {e!s}"
                )
                return
            ...
            error_message = f"Crawling failed: {e!s}"
            safe_logfire_error(
...
async def upload_document(
    ...
    try:
        ...
        safe_logfire_error(
            f"Failed to start document upload | error={e!s} | filename={file.filename} | error_type={type(e).__name__}"
        )
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def _perform_upload_with_progress(
    ...
    try:
        ...
        try:
            ...
            await error_crawl_progress(progress_id, f"Failed to extract text: {e!s}")
            return

        # Use DocumentStorageService to handle the upload
        doc_storage_service = DocumentStorageService(get_database_client())

        # Generate source_id from filename
        source_id = (
            f"file_{filename.replace(' ', '_').replace('.', '_')}_{int(time.time())}"
        )
        ...
        async def document_progress_callback(
            ...
            mapped_percentage = progress_mapper.map_progress(
                "document_storage", percentage
            )
        ...
        if success:
            ...
            safe_logfire_info(
                f"Document uploaded successfully | progress_id={progress_id} | "
        ...
        safe_logfire_error(
            f"Document upload failed | progress_id={progress_id} | filename={file_metadata.get('filename', 'unknown')} | error={e!s}"
        )
        await error_crawl_progress(progress_id, error_msg)
    finally:
        # Clean up task from registry when done (success or failure)
...
async def perform_rag_query(request: RagQueryRequest):
    ...
    try:
        ...
        search_service = RAGService(get_database_client())
        ...
        safe_logfire_error(
            f"RAG query failed | error={e!s} | query={request.query[:50]} | source={request.source}"
        )
        raise HTTPException(
            status_code=500, detail={"error": f"RAG query failed: {e!s}"}
        )
...
async def search_code_examples(request: RagQueryRequest):
    ...
    try:
        ...
        safe_logfire_error(
            f"Code examples search failed | error={e!s} | query={request.query[:50]} | source={request.source}"
        )
        raise HTTPException(
            status_code=500, detail={"error": f"Code examples search failed: {e!s}"}
        )
...
async def get_available_sources():
    ...
    try:
        ...
        safe_logfire_error(f"Failed to get available sources | error={e!s}")
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def delete_source(source_id: str):
    ...
    try:
        ...
        safe_logfire_error(
            f"Failed to delete source | error={e!s} | source_id={source_id}"
        )
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def get_database_metrics():
    ...
    try:
        ...
        service = DatabaseMetricsService(get_database_client())
        metrics = await service.get_metrics()
        return metrics
    except Exception as e:
        safe_logfire_error(f"Failed to get database metrics | error={e!s}")
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def get_crawl_task_status(task_id: str):
    ...
    try:
        ...
        safe_logfire_error(
            f"Failed to get task status | error={e!s} | task_id={task_id}"
        )
        raise HTTPException(status_code=500, detail={"error": str(e)})
...
async def stop_crawl_task(progress_id: str):
    ...
    try:
        ...
        safe_logfire_error(
            f"Failed to stop crawl task | error={e!s} | progress_id={progress_id}"
        )
        raise HTTPException(status_code=500, detail={"error": str(e)})
