...
def extract_text_from_document(
    ...
    try:
        ...
        raise Exception(f"Failed to extract text from {filename}: {e!s}")
...
def extract_text_from_pdf(file_content: bytes) -> str:
    ...
    if PYPDF2_AVAILABLE:
        try:
            ...
            raise Exception(f"PyPDF2 failed to extract text: {e!s}")

    # If we get here, no libraries worked
    raise Exception(
        "Failed to extract text from PDF - no working PDF libraries available"
    )
...
def extract_text_from_docx(file_content: bytes) -> str:
    ...
    if not DOCX_AVAILABLE:
        raise Exception(
            "python-docx library not available. Please install python-docx."
        )
    ...
    try:
        ...
        raise Exception(f"Failed to extract text from Word document: {e!s}")
