...
class LoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            # Log the response
            self.logger.info(
                f"HTTP Response | method={request.method} | path={request.url.path} | "
            ...
            duration = time.time() - start_time
            self.logger.error(
                f"HTTP Error | method={request.method} | path={request.url.path} | error={e!s} | duration_ms={round(duration * 1000, 2)}"
            )
            raise
    ...
    def get_route_handler(self) -> Callable:
        ...
        async def custom_route_handler(request: Request) -> Response:
            ...
            try:
                ...
                # Log endpoint error
                logger.error(
                    f"Endpoint: {endpoint_name} | duration_ms={round(duration * 1000, 2)} | status=error | error={e!s}"
                )
                raise
