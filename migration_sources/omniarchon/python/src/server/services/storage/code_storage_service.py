...
from server.services.embeddings.contextual_embedding_service import (
    generate_contextual_embeddings_batch,
)
from server.services.embeddings.embedding_service import create_embeddings_batch
...
def _normalize_code_for_comparison(code: str) -> str:
    ...
    normalized = re.sub(
        r"from typing_extensions import", "from typing import", normalized
    )
    normalized = re.sub(r"from typing import Annotated[^,\n]*,?", "", normalized)
    normalized = re.sub(
        r"from typing_extensions import Annotated[^,\n]*,?", "", normalized
    )
...
def extract_code_blocks(
    markdown_content: str, min_length: int = None
) -> list[dict[str, Any]]:
    """
    ...
    search_logger.debug(
        f"Extracting code blocks with minimum length: {min_length} characters"
    )
    code_blocks = []
    ...
    if content.startswith("```"):
        ...
        if re.match(r"^```[A-Z]`$", first_line):
            search_logger.warning(
                f"Detected corrupted markdown with fake language: {first_line}"
            )
    ...
    while i < len(backtick_positions) - 1:
        ...
        if not language or language in ["text", "plaintext", "txt"]:
            ...
            if enable_prose_filtering:
                ...
                if len(words) > 0:
                    ...
                    if doc_ratio > max_prose_ratio:
                        search_logger.debug(
                            f"Skipping documentation text disguised as code | doc_ratio={doc_ratio:.2f} | threshold={max_prose_ratio} | first_50_chars={code_content[:50]!r}"
                        )
                        i += 2
                        continue
            ...
            code_pattern_count = sum(
                1 for pattern in code_patterns if pattern in code_content
            )
    ...
    search_logger.debug(
        f"Starting deduplication process for {len(code_blocks)} code blocks"
    )
    ...
    for i, block1 in enumerate(code_blocks):
        ...
        for j, block2 in enumerate(code_blocks):
            ...
            if similarity >= similarity_threshold:
                ...
                search_logger.debug(
                    f"Found similar code blocks with {similarity:.2f} similarity"
                )
...
"""
    ...
    try:
        ...
        search_logger.debug(f"OpenAI API response: {response_content[:200]!r}...")
        ...
        search_logger.error(
            f"Error generating code example summary: {e}, Model: {model_choice}"
        )
        return {
            "example_name": f"Code Example{f' ({language})' if language else ''}",
            "summary": "Code example for demonstration purposes.",
...
async def generate_code_summaries_batch(
    ...
    if max_workers is None:
        try:
            ...
            search_logger.debug(f"Failed to get CODE_SUMMARY_MAX_WORKERS setting: {e}")
    ...
    async def generate_single_summary_with_limit(
        block: dict[str, Any],
    ) -> dict[str, str]:
            ...
            async with lock:
                ...
                if progress_callback:
                    ...
                    progress_percentage = int(
                        (completed_count / len(code_blocks)) * 100
                    )
    ...
    try:
        ...
        for i, summary in enumerate(summaries):
            if isinstance(summary, Exception):
                search_logger.error(
                    f"Error generating summary for code block {i}: {summary}"
                )
        ...
        search_logger.info(
            f"Successfully generated {len(final_summaries)} code summaries"
        )
...
async def add_code_examples_to_supabase(
    ...
    try:
        ...
        use_contextual_embeddings = credential_service._cache.get(
            "USE_CONTEXTUAL_EMBEDDINGS"
        )
        if isinstance(use_contextual_embeddings, str):
            use_contextual_embeddings = use_contextual_embeddings.lower() == "true"
        elif isinstance(
            use_contextual_embeddings, dict
        ) and use_contextual_embeddings.get("is_encrypted"):
            ...
            if encrypted_value:
                try:
                    ...
                    search_logger.debug(
                        f"Failed to decrypt USE_CONTEXTUAL_EMBEDDINGS: {decrypt_err}"
                    )
        ...
        search_logger.debug(f"Failed to get USE_CONTEXTUAL_EMBEDDINGS setting: {e}")
        use_contextual_embeddings = (
    ...
    for i in range(0, total_items, batch_size):
        ...
        for j in range(i, batch_end):
            ...
            code = (
                code_examples[j]
                if isinstance(code_examples[j], str)
                else str(code_examples[j])
            )
            summary = (
                summaries[j] if isinstance(summaries[j], str) else str(summaries[j])
            )
        ...
        for j, (embedding, text) in enumerate(
            ...
            if orig_idx is None:
                search_logger.warning(
                    "Could not map embedding back to original code example"
                )
                continue
            ...
            try:
                ...
                if retry < max_retries - 1:
                    ...
                    for record in batch_data:
                        try:
                            client.table("archon_code_examples").insert(
                                record
                            ).execute()
