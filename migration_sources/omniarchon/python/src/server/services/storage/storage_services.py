...
class DocumentStorageService(BaseStorageService):
    ...
    async def upload_document(
        ...
        """
        logger.info(
            f"Document upload starting: {filename} as {knowledge_type} knowledge"
        )
        ...
        with safe_span(
            ...
            try:
                ...
                logger.info(
                    f"Updating source info for {source_id} with knowledge_type={knowledge_type}"
                )
                ...
                return False, {"error": f"Error uploading document: {e!s}"}
