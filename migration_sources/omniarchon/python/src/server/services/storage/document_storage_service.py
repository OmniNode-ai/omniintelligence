...
Handles storage of documents in database with parallel processing support.
"""
...
from server.config.logfire_config import safe_span, search_logger
...
async def add_documents_to_database(
    ...
    search_logger.info("🔍 DEBUG: add_documents_to_database called with:")
    search_logger.info(f"  - Total contents: {len(contents)}")
    search_logger.info(f"  - Total URLs: {len(urls)}")
    search_logger.info(f"  - Total chunk_numbers: {len(chunk_numbers)}")
    search_logger.info(f"  - Total metadatas: {len(metadatas)}")
    search_logger.info(f"  - url_to_full_document keys: {len(url_to_full_document)}")
    search_logger.info(f"  - First 3 URLs: {urls[:3] if urls else 'None'}")
    search_logger.info(
        f"  - First content preview: {contents[0][:100] + '...' if contents else 'None'}"
    )
    search_logger.info(f"  - First metadata: {metadatas[0] if metadatas else 'None'}")
    ...
    with safe_span(
        ...
        try:
            ...
            enable_parallel_batches = (
                rag_settings.get("ENABLE_PARALLEL_BATCHES", "true").lower() == "true"
            )
        except Exception as e:
            search_logger.warning(
                f"Failed to load storage settings: {e}, using defaults"
            )
            if batch_size is None:
                batch_size = 50
            ...
            if unique_urls:
                ...
                for i in range(0, len(unique_urls), delete_batch_size):
                    ...
                    if i + delete_batch_size < len(unique_urls):
                        await asyncio.sleep(
                            0.05
                        )  # Reduced pause between delete batches
                search_logger.info(
        ...
        try:
            ...
            search_logger.debug(
                f"Failed to get USE_CONTEXTUAL_EMBEDDINGS credential: {e}"
            )
        ...
        for batch_num, i in enumerate(range(0, len(contents), batch_size), 1):
            ...
            if use_contextual_embeddings:
                try:
                    ...
                    search_logger.debug(
                        f"Failed to get CONTEXTUAL_EMBEDDINGS_MAX_WORKERS credential: {e}"
                    )
                    max_workers = 4
            else:
                max_workers = 1
            ...
            if use_contextual_embeddings:
                ...
                try:
                    ...
                    search_logger.debug(
                        f"Failed to parse CONTEXTUAL_EMBEDDING_BATCH_SIZE setting: {e}"
                    )
            ...
            result = await create_embeddings_batch(
                ...
                provider=provider,
                progress_callback=(
                    embedding_progress_wrapper if progress_callback else None
                ),
            )
                ...
                try:
                    ...
                    if retry < max_retries - 1:
                        ...
                        for record in batch_data:
                            ...
                            try:
                                client.table("archon_crawled_pages").insert(
                                    record
                                ).execute()
