...
class KnowledgeItemService:
        ...
        try:
            ...
            for source in sources:
                ...
                source_type = self._determine_source_type(
                    source_metadata, first_page_url
                )
                ...
                item = {
                    ...
                    "source_id": source_id,
                    "code_examples": (
                        [{"count": code_examples_count}]
                        if code_examples_count > 0
                        else []
                    ),  # Minimal array just for count display
                    "metadata": {
                        "knowledge_type": source_metadata.get(
                        ...
                        "estimated_pages": round(
                            source.get("total_word_count", 0) / 250, 1
                        ),
            ...
            safe_logfire_error(f"Failed to list knowledge items | error={e!s}")
            raise
    ...
    async def get_item(self, source_id: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            safe_logfire_error(
                f"Failed to get knowledge item | error={e!s} | source_id={source_id}"
            )
            return None
    ...
    async def update_item(
        ...
        try:
            ...
            metadata_updates = {
                k: v for k, v in updates.items() if k in metadata_fields
            }
            ...
            if result.data:
                safe_logfire_info(
                    f"Knowledge item updated successfully | source_id={source_id}"
                )
                return True, {
                    "success": True,
    ...
    def _filter_by_search(
        self, items: list[dict[str, Any]], search: str
    ) -> list[dict[str, Any]]:
        ...
        return [
            ...
            or any(
                search_lower in tag.lower() for tag in item["metadata"].get("tags", [])
            )
        ]
