...
class ProcessingMode(str, Enum):
    ...
    WEBSOCKET_SAFE = (
        "websocket_safe"  # Operations that need to yield for WebSocket health
    )
...
class RateLimiter:
    ...
    async def acquire(
        self, estimated_tokens: int = 8000, progress_callback: Callable | None = None
    ) -> bool:
    ...
    def _can_make_request(self, estimated_tokens: int) -> bool:
        ...
        return not current_tokens + estimated_tokens > self.config.tokens_per_minute
...
class MemoryAdaptiveDispatcher:
    ...
    def calculate_optimal_workers(
        self, mode: ProcessingMode = ProcessingMode.CPU_INTENSIVE
    ) -> int:
...
class ThreadingService:
    ...
    async def start(self):
        ...
        logfire_logger.info(
            "Threading service started", extra={"config": self.config.__dict__}
        )
    ...
    async def rate_limited_operation(
        ...
        async with self.rate_limiter.semaphore:
            can_proceed = await self.rate_limiter.acquire(
                estimated_tokens, progress_callback
            )
            if not can_proceed:
            ...
            try:
                ...
                logfire_logger.info(
                    ...
                    extra={
                        ...
                        "active_websockets": len(
                            self.websocket_processor.active_connections
                        ),
                    },
                )
