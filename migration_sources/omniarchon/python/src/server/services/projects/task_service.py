...
try:
    ...
    logger.warning(
        f"❌ Socket.IO broadcasting not available - Exception: {type(e).__name__}: {e}"
    )
    import traceback
...
class TaskService:
        ...
        try:
            ...
            if task_order > 0:
                ...
                if existing_tasks_response.data:
                    logger.info(
                        f"Reordering {len(existing_tasks_response.data)} existing tasks"
                    )
            ...
            # Debug logging for parent_task_id
            logger.info(f"About to insert task with parent_task_id: {parent_task_id}")
            logger.info(f"Full task_data being inserted: {task_data}")

            response = (
                self.supabase_client.table("archon_tasks").insert(task_data).execute()
            )

            # Debug logging for response
            logger.info(f"Database response data: {response.data}")
            if response.data and len(response.data) > 0:
                logger.info(
                    f"Inserted task parent_task_id: {response.data[0].get('parent_task_id')}"
                )
            ...
            if response.data:
                ...
                if _broadcast_available:
                    try:
                        ...
                        logger.info(
                            f"Socket.IO broadcast sent for new task {task['id']}"
                        )
            ...
            return False, {"error": f"Error creating task: {e!s}"}
    ...
    def list_tasks(
        ...
        try:
            ...
            response = (
                query.order("task_order", desc=False)
            ...
            return False, {"error": f"Error listing tasks: {e!s}"}
    ...
    def get_task(self, task_id: str) -> tuple[bool, dict[str, Any]]:
        ...
        try:
            ...
            return False, {"error": f"Error getting task: {e!s}"}
    ...
    async def update_task(
        ...
        try:
            ...
            if response.data:
                ...
                if _broadcast_available:
                    try:
                        ...
                        logger.info(
                            f"✅ Socket.IO broadcast successful for task {task_id}"
                        )
                    except Exception as ws_error:
            ...
            return False, {"error": f"Error updating task: {e!s}"}
    ...
    async def archive_task(
        ...
        try:
            ...
            if response.data:
                ...
                if _broadcast_available:
                    try:
                        ...
                        logger.info(
                            f"Socket.IO broadcast sent for archived task {task_id}"
                        )
                    except Exception as ws_error:
                        logger.warning(
            ...
            return False, {"error": f"Error archiving task: {e!s}"}

    # Task Hierarchy Query Functions

    async def get_child_tasks(self, parent_task_id: str) -> tuple[bool, dict[str, Any]]:
        """
        ...
        Returns:
            Tuple of (success, result_dict with child_tasks list)
        """
        try:
            response = (
                self.supabase_client.table("archon_tasks")
            ...
            child_tasks = response.data or []

            return True, {
                "child_tasks": child_tasks,
                "count": len(child_tasks),
                "parent_task_id": parent_task_id,
            }

        except Exception as e:
            logger.error(f"Error getting child tasks for {parent_task_id}: {e}")
            return False, {"error": f"Error getting child tasks: {e!s}"}

    async def get_parent_task(self, task_id: str) -> tuple[bool, dict[str, Any]]:
        """
        Get the parent task of a given task.

        Args:
            task_id: UUID of the child task

        Returns:
            Tuple of (success, result_dict with parent_task or None)
        """
        try:
            # First get the task to find its parent_task_id
            task_response = (
                self.supabase_client.table("archon_tasks")
            ...
            parent_task_id = task_response.data[0].get("parent_task_id")

            if not parent_task_id:
                return True, {"parent_task": None, "message": "Task has no parent"}
            ...
            if parent_response.data:
                return True, {"parent_task": parent_response.data[0]}
            else:
                return False, {"error": f"Parent task {parent_task_id} not found"}

        except Exception as e:
            logger.error(f"Error getting parent task for {task_id}: {e}")
            return False, {"error": f"Error getting parent task: {e!s}"}

    async def get_task_hierarchy(
        self, task_id: str, max_depth: int = 5
    ) -> tuple[bool, dict[str, Any]]:
        """
        Get the complete task hierarchy tree for a given task.

        Args:
            task_id: UUID of the root task
            max_depth: Maximum depth to traverse (prevents infinite loops)

        Returns:
            Tuple of (success, result_dict with hierarchy tree)
        """
        try:

            async def build_hierarchy(
                current_task_id: str, depth: int = 0
            ) -> dict[str, Any] | None:
                if depth >= max_depth:
                    return None
                ...
                # Get child tasks
                children_success, children_result = await self.get_child_tasks(
                    current_task_id
                )

                task_node = {**task, "children": [], "depth": depth}

                if children_success and children_result.get("child_tasks"):
                    for child in children_result["child_tasks"]:
                        child_hierarchy = await build_hierarchy(child["id"], depth + 1)
                        if child_hierarchy:
                            task_node["children"].append(child_hierarchy)

                return task_node

            hierarchy = await build_hierarchy(task_id)

            if hierarchy:
                return True, {"hierarchy": hierarchy, "root_task_id": task_id}
            else:
                return False, {"error": f"Could not build hierarchy for task {task_id}"}

        except Exception as e:
            logger.error(f"Error getting task hierarchy for {task_id}: {e}")
            return False, {"error": f"Error getting task hierarchy: {e!s}"}
