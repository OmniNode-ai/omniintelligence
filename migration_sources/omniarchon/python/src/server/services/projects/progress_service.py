...
class ProgressService:
    ...
    def start_operation(
        ...
        logger.info(
            f"🎬 [PROGRESS] Started tracking {operation_type} operation: {progress_id}"
        )
        logger.info(
    ...
    async def update_progress(
        self, progress_id: str, update_data: dict[str, Any]
    ) -> None:
        """
        ...
        if progress_id not in self.active_operations:
            logger.warning(
                f"📊 [PROGRESS] Attempted to update unknown operation: {progress_id}"
            )
            logger.warning(
                f"📊 [PROGRESS] Active operations: {list(self.active_operations.keys())}"
            )
        ...
        if "log" in update_data:
            ...
            if len(self.active_operations[progress_id]["logs"]) > 50:
                self.active_operations[progress_id]["logs"] = self.active_operations[
                    progress_id
                ]["logs"][-50:]
    ...
    async def complete_operation(
        self, progress_id: str, completion_data: dict[str, Any]
    ) -> None:
        """
    ...
    async def _broadcast_progress(self, progress_id: str) -> None:
        ...
        if "start_time" in progress_data and hasattr(
            progress_data["start_time"], "isoformat"
        ):
        ...
        try:
            ...
            logger.error(
                f"❌ [PROGRESS] Error broadcasting progress via Socket.IO: {e}"
            )
