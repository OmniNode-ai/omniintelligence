...
class AgenticRAGStrategy:
    ...
    def __init__(self, supabase_client: Client, base_strategy):
        ...
        Args:
            database_client: Database client for database operations
        ...
        try:
            ...
            if (
                ...
                if cached_value:
                    ...
                    if isinstance(cached_value, dict) and cached_value.get(
                        "is_encrypted"
                    ):
                        encrypted_value = cached_value.get("encrypted_value")
                        if encrypted_value:
                            try:
                                value = credential_service._decrypt_value(
                                    encrypted_value
                                )
                            except Exception:
                                return False
    ...
    def _extract_code_context(self, result: dict[str, Any]) -> dict[str, Any]:
        ...
        if isinstance(metadata, dict):
            ...
            if "line_start" in metadata and "line_end" in metadata:
                context["line_range"] = (
                    f"{metadata['line_start']}-{metadata['line_end']}"
                )
    ...
    def analyze_code_query(self, query: str) -> dict[str, Any]:
        ...
        is_code_query = (
            len(detected_languages) > 0
            or len(detected_frameworks) > 0
            or len(code_indicators) > 0
        )
...
async def search_code_examples_agentic(
    ...
    return await strategy.search_code_examples_async(
        query, match_count, filter_metadata, source_id
    )
