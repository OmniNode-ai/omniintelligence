...
from server.services.search.enhanced_search_strategy import EnhancedSearchStrategy
...
class RAGService:
    ...
    def __init__(self, supabase_client=None):
        ...
        self.hybrid_strategy = HybridSearchStrategy(
            self.supabase_client, self.base_strategy
        )
        self.agentic_strategy = AgenticRAGStrategy(
            self.supabase_client, self.base_strategy
        )

        # Initialize enhanced search strategy
        self.enhanced_strategy = EnhancedSearchStrategy(
            self.supabase_client, self.base_strategy
        )
    ...
    async def close(self):
        """Clean up resources, including HTTP clients from strategies"""
        try:
            if hasattr(self.enhanced_strategy, "close"):
                await self.enhanced_strategy.close()
    ...
    def get_setting(self, key: str, default: str = "false") -> str:
        ...
        try:
            ...
            if (
                hasattr(credential_service, "_cache")
                and credential_service._cache_initialized
            ):
        ...
        with safe_span(
            ...
            try:
                ...
                if self.reranking_strategy and formatted_results:
                    try:
                        formatted_results = (
                            await self.reranking_strategy.rerank_results(
                                query, formatted_results, content_key="content"
                            )
                        )
                        reranking_applied = True
                        logger.debug(
                            f"Reranking applied to {len(formatted_results)} results"
                        )
                    except Exception as e:
                ...
                logger.info(
                    f"RAG query completed - {len(formatted_results)} results found"
                )
    ...
    async def search_code_examples_service(
        ...
        """
        Search for code examples using agentic strategy with hybrid search
        and reranking.
        ...
        with safe_span(
            ...
            try:
                ...
                if not self.agentic_strategy.is_enabled():
                    return False, {
                        "error": "Code example extraction is disabled. "
                        "Enable USE_AGENTIC_RAG setting to use this feature.",
                        "query": query,
                    }
                ...
                # Prepare filter
                filter_metadata = (
                    {"source": source_id} if source_id and source_id.strip() else None
                )
    ...
    async def enhanced_search_service(
        self,
        query: str,
        ...
        **kwargs,
        ...
        """
        Enhanced search using the new hybrid search service with quality scores.
        ...
        Returns:
            Tuple of (success, result_dict) with enhanced search results
        """
        with safe_span(
            ...
            try:
                logger.info(
                    f"Enhanced search started: {query[:100]}"
                    f"{'...' if len(query) > 100 else ''}"
                )

                if include_analytics:
                    # Use analytics-enabled search
                    result = await self.enhanced_strategy.search_with_analytics(
                        query=query,
                        match_count=match_count,
                        search_mode=search_mode,
                    ...
                    span.set_attribute("analytics_included", True)
                    span.set_attribute(
                        "search_effectiveness",
                        result.get("analytics", {}).get("search_effectiveness", 0.0),
                    )

                    return result["success"], result
                else:
                    # Use standard enhanced search
                    results = await self.enhanced_strategy.enhanced_search(
                        query=query,
                        match_count=match_count,
                        search_mode=search_mode,
                    ...
                    response_data = {
                        ...
                        "execution_path": "enhanced_search_service",
                        "enhanced_features": {
                            "quality_scores": any(
                                "quality_score" in r for r in results
                            ),
                            "multi_dimensional": any(
                                "semantic_score" in r for r in results
                            ),
                            "composite_ranking": any(
                                "composite_score" in r for r in results
                            ),
                        },
                    }

                    span.set_attribute("results_returned", len(results))
                    span.set_attribute("enhanced_features_used", True)

                    logger.info(
                        f"Enhanced search completed - {len(results)} results found"
                    )
                    return True, response_data

            except Exception as e:
                logger.error(f"Enhanced search service failed: {e}")
                span.set_attribute("error", str(e))
                span.set_attribute("success", False)
