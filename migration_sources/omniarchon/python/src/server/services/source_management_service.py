...
from server.config.logfire_config import get_logger, search_logger
...
def extract_source_summary(
    ...
    search_logger.info(
        f"Generating summary for {source_id} using model: {model_choice}"
    )
...
"""
    ...
    try:
        try:
            ...
            search_logger.info(
                "Successfully created LLM client fallback for summary generation"
            )
        except Exception as e:
            ...
            try:
                ...
                if not api_key:
                    ...
                    if (
                        ...
                        cached_key = credential_service._cache["OPENAI_API_KEY"]
                        if isinstance(cached_key, dict) and cached_key.get(
                            "is_encrypted"
                        ):
                            api_key = credential_service._decrypt_value(
                                cached_key["encrypted_value"]
...
def update_source_info(
    ...
    """
    search_logger.info(
        f"Updating source {source_id} with knowledge_type={knowledge_type}"
    )
    try:
        ...
        if existing_source.data:
            ...
            search_logger.info(
                f"Preserving existing title for {source_id}: {existing_title}"
            )

            # Update metadata while preserving title
            # Determine source_type based on source_url or original_url
            if (source_url and source_url.startswith("file://")) or (
                original_url and original_url.startswith("file://")
            ):
                source_type = "file"
            else:
                source_type = "url"
            ...
            search_logger.info(
                f"Updating existing source {source_id} metadata: knowledge_type={knowledge_type}"
            )
            if original_url:
            ...
            search_logger.info(
                f"Creating new source {source_id} with knowledge_type={knowledge_type}"
            )
            ...
            search_logger.info(
                f"Created/updated source {source_id} with title: {title}"
            )
...
class SourceManagementService:
    ...
    def get_available_sources(self) -> tuple[bool, dict[str, Any]]:
        ...
        try:
            ...
            return False, {"error": f"Error retrieving sources: {e!s}"}
    ...
    def delete_source(self, source_id: str) -> tuple[bool, dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                return False, {
                    "error": f"Failed to delete crawled pages: {pages_error!s}"
                }
            ...
            try:
                ...
                return False, {
                    "error": f"Failed to delete code examples: {code_error!s}"
                }
            ...
            try:
                ...
                source_deleted = (
                    len(source_response.data) if source_response.data else 0
                )
                logger.info(f"Deleted {source_deleted} source records")
            except Exception as source_error:
                logger.error(f"Failed to delete from sources: {source_error}")
                return False, {"error": f"Failed to delete source: {source_error!s}"}
            ...
            return False, {"error": f"Error deleting source: {e!s}"}
    ...
    def update_source_metadata(
        ...
        try:
            ...
            return False, {"error": f"Error updating source metadata: {e!s}"}
    ...
    def create_source_info(
        ...
        try:
            ...
            return False, {"error": f"Error creating source info: {e!s}"}
    ...
    def get_source_details(self, source_id: str) -> tuple[bool, dict[str, Any]]:
        ...
        try:
            ...
            return False, {"error": f"Error getting source details: {e!s}"}

    def list_sources_by_type(
        self, knowledge_type: str = None
    ) -> tuple[bool, dict[str, Any]]:
        """
        ...
        try:
            ...
            return False, {"error": f"Error listing sources by type: {e!s}"}
