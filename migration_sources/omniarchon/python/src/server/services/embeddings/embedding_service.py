...
from server.services.embeddings.embedding_exceptions import (
    EmbeddingAPIError,
    EmbeddingError,
    EmbeddingQuotaExhaustedError,
    EmbeddingRateLimitError,
)
from server.services.llm_provider_service import get_embedding_model, get_llm_client
from server.services.threading_service import get_threading_service
    ...
    with safe_span(
        ...
        try:
            async with get_llm_client(
                provider=provider, use_embedding_provider=True
            ) as client:
                            ...
                            while retry_count < max_retries:
                                try:
                                    ...
                                    for text, item in zip(
                                        batch, response.data, strict=False
                                    ):
                        ...
                        for text in batch:
                            if isinstance(e, EmbeddingError):
                                ...
                                result.add_failure(
                                    text,
                                    EmbeddingAPIError(
                                        f"Failed to create embedding: {e!s}",
                                        original_error=e,
                                    ),
                                    batch_index,
                                )
            ...
            for text in texts[processed_count:]:
                result.add_failure(
                    text,
                    EmbeddingAPIError(f"Catastrophic failure: {e!s}", original_error=e),
                )
