...
from server.config.logfire_config import safe_logfire_error, safe_logfire_info
...
class CodeExtractionService:
    ...
    def __init__(self, supabase_client):
        ...
        Args:
            database_client: The database client for database operations
        """
    ...
    async def _get_setting(self, key: str, default: Any) -> Any:
        ...
        try:
            ...
            safe_logfire_error(
                f"Error getting setting {key}: {e}, using default: {default}"
            )
    ...
    async def _extract_code_blocks_from_documents(
        ...
        for doc in crawl_results:
            try:
                ...
                (md[:1000] if md else html_content[:1000] if html_content else "")
                ...
                if md:
                    ...
                    if "getting-started" in source_url and md:
                        ...
                        sample = md[:500]
                        safe_logfire_info(
                            f"Markdown sample for getting-started: {sample}..."
                        )
                ...
                if is_text_file:
                    ...
                    if text_content:
                        ...
                        safe_logfire_info(
                            f"🔍 Content preview (first 500 chars): {text_content[:500]!r}..."
                        )
                        ...
                        safe_logfire_info(
                            f"⚠️ NO CONTENT for text file | url={source_url}"
                        )
                ...
                if len(code_blocks) == 0 and html_content and not is_text_file:
                    ...
                    html_code_blocks = await self._extract_html_code_blocks(
                        html_content
                    )
                    if html_code_blocks:
                ...
                safe_logfire_error(
                    f"Error processing code from document | url={doc.get('url')} | error={e!s}"
                )
    ...
    async def _extract_html_code_blocks(self, content: str) -> list[dict[str, Any]]:
        ...
        safe_logfire_info(
            f"Processing HTML of length {len(content)} for code extraction"
        )

        # Check if we have actual content
        if len(content) < 1000:
            safe_logfire_info(
                f"Warning: HTML content seems too short, first 500 chars: {content[:500]!r}"
            )
        ...
        if pre_matches:
            safe_logfire_info(
                f"Found {len(pre_matches)} <pre> tags in first 5000 chars"
            )
        ...
        for pattern_tuple in patterns:
            ...
            for match in matches:
                ...
                context_for_length = content[
                    max(0, code_start_pos - 500) : code_start_pos + 500
                ]
                min_length = await self._calculate_min_length(
                    language, context_for_length
                )
                ...
                if len(code_content) < min_length:
                    ...
                    if language and code_start_pos > 0:
                        ...
                        (
                            complete_code,
                            block_end_pos,
                        ) = await self._find_complete_code_block(
                ...
                if not overlapping:
                    ...
                    context_after = content[
                        end_pos : min(len(content), end_pos + 1000)
                    ].strip()
    ...
    async def _extract_text_file_code_blocks(
        ...
        safe_logfire_info(f"📄 First 1000 chars: {content[:1000]!r}...")
        safe_logfire_info(
            f"📄 Sample showing backticks: {content[5000:6000]!r}..."
            if len(content) > 6000
        ...
        for i, match in enumerate(matches):
            ...
            context_around = content[
                max(0, start_pos - 500) : min(len(content), end_pos + 500)
            ]
            if min_length is None:
                actual_min_length = await self._calculate_min_length(
                    language, context_around
                )
            else:
            ...
            if len(code_content) >= actual_min_length:
                ...
                context_after = content[
                    end_pos : min(len(content), end_pos + 500)
                ].strip()
        ...
        if len(code_blocks) == 0:  # Only if we haven't found code blocks yet
            ...
            for i, line in enumerate(lines):
                ...
                if indent >= 4 and stripped:  # At least 4 spaces and not empty
                    ...
                    # Get context
                    context_before_lines = lines[
                        max(0, block_start_idx - 10) : block_start_idx
                    ]
    ...
    async def _calculate_min_length(self, language: str, context: str) -> int:
        ...
        if any(
            word in context_lower for word in ["example", "snippet", "sample", "demo"]
        ):
            min_length = int(min_length * 0.7)  # Examples can be shorter
        elif any(
            word in context_lower for word in ["implementation", "complete", "full"]
        ):
    ...
    def _clean_code_content(self, code: str, language: str = "") -> str:
        ...
        spacing_fixes = [
            ...
            # Control flow
            (
                r"(\b(?:if|elif|else|for|while|try|except|finally|with)\b)([A-Za-z])",
                r"\1 \2",
            ),
        ...
        if language.lower() in ["python", "py"]:
            ...
            code = re.sub(
                r"(\b(?:from|import)\b)(\w+)(\b(?:import)\b)", r"\1 \2 \3", code
            )
            # Fix missing colons
            code = re.sub(
    ...
    async def _validate_code_quality(self, code: str, language: str = "") -> bool:
        ...
        for pattern in bad_patterns:
            if re.search(pattern, code):
                safe_logfire_info(
                    f"Code failed quality check: pattern '{pattern}' found"
                )
                return False
        ...
        if language.lower() in self.LANGUAGE_PATTERNS:
            ...
            if (
                found_lang_indicators < 2
            ):  # Need at least 2 language-specific indicators
                safe_logfire_info(
        ...
        if len(non_empty_lines) < 3:
            safe_logfire_info(
                f"Code has too few non-empty lines: {len(non_empty_lines)}"
            )
            return False
    ...
    async def _generate_code_summaries(
        ...
        if not await self._is_code_summaries_enabled():
            safe_logfire_info(
                "Code summaries generation is disabled, returning default summaries"
            )
    ...
    def _prepare_code_examples_for_storage(
        ...
        for code_item, summary_result in zip(
            all_code_blocks, summary_results, strict=False
        ):
    ...
    async def _store_code_examples(
        ...
        try:
            ...
            safe_logfire_info(
                f"Successfully stored {len(storage_data['examples'])} code examples"
            )
            return len(storage_data["examples"])

        except Exception as e:
            safe_logfire_error(f"Error storing code examples | error={e!s}")
            return 0
