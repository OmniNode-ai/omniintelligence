...
class URLHandler:
        ...
        try:
            ...
            for ext in binary_extensions:
                if path.endswith(ext):
                    logger.debug(
                        f"Skipping binary file: {url} (matched extension: {ext})"
                    )
                    return True
    ...
    def generate_unique_source_id(url: str) -> str:
        ...
        try:
            ...
            logger.error(
                f"Error generating unique source ID for {redacted}: {e}", exc_info=True
            )

            # Fallback: use a hash of the error message + url to still get something unique
            fallback = f"error_{redacted}_{e!s}"
    ...
    def extract_display_name(url: str) -> str:
        ...
        try:
            ...
            if domain.startswith("docs."):
                ...
                base_name = (
                    f"{service_name.title()}" if service_name else "Documentation"
                )
                ...
                if "sitemap" in path.lower() and path.endswith(".xml"):
                    ...
                    for tld in [".com", ".org", ".io", ".dev", ".net", ".ai", ".app"]:
                        if display.endswith(tld):
                            ...
                            break
                    display_parts = (
                        display.replace("-", " ").replace("_", " ").split(".")
                    )
