...
from server.services.storage.document_storage_service import add_documents_to_database
...
class DocumentStorageOperations:
    ...
    async def process_and_store_documents(
        ...
        # ADD DEBUG LOGGING FOR CRAWL RESULTS
        safe_logfire_info(f"🔍 DEBUG: Processing {len(crawl_results)} crawl results")
        for i, result in enumerate(crawl_results[:2]):  # Log first 2 results
            safe_logfire_info(
                f"  Crawl Result {i}: URL={result.get('url', 'NO_URL')}, has_markdown={bool(result.get('markdown'))}, markdown_length={len(result.get('markdown', ''))}"
            )
        ...
        for doc_index, doc in enumerate(crawl_results):
            ...
            safe_logfire_info(
                f"Using original source_id '{source_id}' for URL '{doc_url}'"
            )
            ...
            for i, chunk in enumerate(chunks):
                ...
                source_word_counts[source_id] = (
                    source_word_counts.get(source_id, 0) + word_count
                )
        ...
        safe_logfire_info(
            f"url_to_full_document keys: {list(url_to_full_document.keys())[:5]}"
        )
        ...
        # ADD DEBUG LOGGING BEFORE CALLING add_documents_to_database
        safe_logfire_info("🔍 DEBUG: About to call add_documents_to_database with:")
        safe_logfire_info(f"  - all_urls length: {len(all_urls)}")
        safe_logfire_info(f"  - all_chunk_numbers length: {len(all_chunk_numbers)}")
        safe_logfire_info(f"  - all_contents length: {len(all_contents)}")
        safe_logfire_info(f"  - all_metadatas length: {len(all_metadatas)}")
        safe_logfire_info(
            f"  - url_to_full_document keys: {list(url_to_full_document.keys())[:3]}"
        )
        safe_logfire_info(f"  - Sample URL: {all_urls[0] if all_urls else 'None'}")
        safe_logfire_info(
            f"  - Sample content (first 100 chars): {all_contents[0][:100] + '...' if all_contents else 'None'}"
        )
        safe_logfire_info(
            f"  - Sample metadata: {all_metadatas[0] if all_metadatas else 'None'}"
        )

        # Call add_documents_to_database with the correct parameters
    ...
    async def _create_source_records(
        ...
        for source_id in unique_source_ids:
            ...
            try:
                ...
                safe_logfire_error(
                    f"Failed to generate AI summary for '{source_id}': {e!s}, using fallback"
                )
            ...
            try:
                ...
                safe_logfire_info(
                    f"Successfully created/updated source record for '{source_id}'"
                )
            except Exception as e:
                safe_logfire_error(
                    f"Failed to create/update source record for '{source_id}': {e!s}"
                )
                ...
                try:
                    ...
                    safe_logfire_info(
                        f"Fallback source creation succeeded for '{source_id}'"
                    )
                except Exception as fallback_error:
                    safe_logfire_error(
                        f"Both source creation attempts failed for '{source_id}': {fallback_error!s}"
                    )
                    raise Exception(
                        f"Unable to create source record for '{source_id}'. This will cause foreign key violations. Error: {fallback_error!s}"
                    )
        ...
        if unique_source_ids:
            for source_id in unique_source_ids:
                try:
                    ...
                    safe_logfire_error(
                        f"Source verification failed for '{source_id}': {e!s}"
                    )
                    raise
