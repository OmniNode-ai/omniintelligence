...
class SinglePageCrawlStrategy:
    ...
    async def crawl_single_page(
        ...
        for attempt in range(retry_count):
            try:
                ...
                if is_doc_site:
                    ...
                    logger.info(
                        f"Detected documentation site, using wait selector: {wait_selector}"
                    )
                ...
                logger.info(
                    f"Using wait_until: {crawl_config.wait_until}, page_timeout: {crawl_config.page_timeout}"
                )

                try:
                    result = await self.crawler.arun(url=url, config=crawl_config)
                except Exception as e:
                    last_error = f"Crawler exception for {url}: {e!s}"
                ...
                has_triple_backticks = (
                    "```" in result.markdown if result.markdown else False
                )
                ...
                if "getting-started" in url:
                    logger.info(
                        f"Markdown sample for getting-started: {markdown_sample}"
                    )
                ...
                last_error = f"Error crawling page: {e!s}"
        ...
        return {
            "success": False,
            "error": last_error
            or f"Failed to crawl {url} after {retry_count} attempts",
        }
        ...
        try:
            ...
            if result.success and result.markdown:
                ...
                await report_progress(
                    end_progress, f"Text file crawled successfully: {original_url}"
                )
