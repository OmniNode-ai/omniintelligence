...
class RecursiveCrawlStrategy:
    ...
    async def crawl_recursive_with_progress(
        ...
        try:
            settings = await credential_service.get_credentials_by_category(
                "rag_strategy"
            )
        ...
        if has_doc_sites:
            ...
            run_config = CrawlerRunConfig(
                ...
                delay_before_return_html=float(
                    settings.get("CRAWL_DELAY_BEFORE_HTML", "1.0")
                ),
            ...
            run_config = CrawlerRunConfig(
                ...
                delay_before_return_html=float(
                    settings.get("CRAWL_DELAY_BEFORE_HTML", "0.5")
                ),
                scan_full_page=True,
            )
        ...
        for depth in range(max_depth):
            ...
            for batch_idx in range(0, len(urls_to_crawl), batch_size):
                ...
                logger.info(
                    f"Starting parallel crawl of {len(batch_urls)} URLs with arun_many"
                )
                batch_results = await self.crawler.arun_many(
                    urls=transformed_batch_urls,
                    config=run_config,
                    ...
                    if result.success and result.markdown:
                        ...
                        for link in links.get("internal", []):
                            ...
                            if next_url not in visited and not is_binary:
                                ...
                                logger.debug(
                                    f"Skipping binary file from crawl queue: {next_url}"
                                )
                    else:
                        logger.warning(
