...
def _ensure_socketio_imports():
    ...
    if update_crawl_progress is None:
        from ...api_routes.socketio_handlers import complete_crawl_progress as _complete
        from ...api_routes.socketio_handlers import update_crawl_progress as _update
...
# Import operations
from server.services.crawling.document_storage_operations import (
...
class CrawlingService:
    ...
    def __init__(self, crawler=None, supabase_client=None, progress_id=None):
        ...
        self.recursive_strategy = RecursiveCrawlStrategy(
            crawler, self.markdown_generator
        )
        ...
        self.progress_state = (
            {"progressId": self.progress_id} if self.progress_id else {}
        )
    ...
    async def _handle_progress_update(
        self, task_id: str, update: dict[str, Any]
    ) -> None:
        """
    ...
    async def _async_orchestrate_crawl(self, request: dict[str, Any], task_id: str):
        ...
        try:
            url = str(request.get("url", ""))
            safe_logfire_info(
                f"Starting async crawl orchestration | url={url} | task_id={task_id}"
            )
            ...
            async def update_mapped_progress(
                ...
                overall_progress = self.progress_mapper.map_progress(
            ...
            safe_logfire_info(
                f"🔍 DEBUG: Crawl completed - type: {crawl_type}, results count: {len(crawl_results) if crawl_results else 0}"
            )
            if crawl_results:
                for i, result in enumerate(crawl_results[:2]):  # Log first 2 results
                    safe_logfire_info(
                        f"  Result {i}: URL={result.get('url', 'NO_URL')}, has_markdown={bool(result.get('markdown'))}, content_length={len(result.get('markdown', ''))}"
                    )
            else:
                safe_logfire_error(
                    "🚨 DEBUG: No crawl results returned from crawl_by_url_type!"
                )
                ...
                async def code_progress_callback(data: dict):
                    if self.progress_id:
                        ...
                        await update_crawl_progress(
                            self.progress_id, self.progress_state
                        )

                code_examples_count = (
                    await self.doc_storage_ops.extract_and_store_code_examples(
                        crawl_results,
            ...
            safe_logfire_info(
                f"Crawl operation cancelled | progress_id={self.progress_id}"
            )
            await self._handle_progress_update(
                task_id,
                {
            ...
            safe_logfire_error(f"Async crawl orchestration failed | error={e!s}")
            await self._handle_progress_update(
                task_id,
                {"status": "error", "percentage": -1, "log": f"Crawl failed: {e!s}"},
            )
            # Unregister on error
            if self.progress_id:
                unregister_orchestration(self.progress_id)
    ...
    async def _crawl_by_url_type(self, url: str, request: dict[str, Any]) -> tuple:
        ...
        if self.url_handler.is_txt(url):
            ...
            crawl_results = await self.crawl_markdown_file(
                url,
                progress_callback=await self._create_crawl_progress_callback(
                    "crawling"
                ),
                start_progress=10,
                end_progress=20,
