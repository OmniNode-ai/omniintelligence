...
class CredentialService:
    ...
    def _get_database_client(self) -> Client:
        """
        Get or create a properly configured database client using environment variables. Uses the standard database client initialization.
        """
        if self._supabase is None:
            ...
            try:
                # Initialize with standard database client - no need for custom headers
                ...
                if match:
                    ...
                    logger.info(
                        f"Database client initialized for project: {project_id}"
                    )
                else:
    ...
    def _get_encryption_key(self) -> bytes:
        ...
        # Use database service key as the basis for encryption key
    ...
    async def load_all_credentials(self) -> dict[str, Any]:
        ...
        Returns empty dict if database unavailable (graceful degradation).
        """
        try:
            supabase = self._get_database_client()
            ...
            # Graceful degradation: Allow service to start without database credentials
            # This enables the system to use environment variables instead
            logger.warning(
                f"Could not load credentials from database: {e!s}. "
                "Service will use environment variables for configuration."
            )
            self._cache = {}
            self._cache_initialized = True  # Mark as initialized to prevent retry loops
            return {}
    ...
    async def get_active_provider(self, service_type: str = "llm") -> dict[str, Any]:
        ...
        try:
            ...
            return {
                "provider": provider,
                "api_key": (
                    os.getenv("OPENAI_API_KEY") if provider == "openai" else "ollama"
                ),
                "base_url": base_url,
                "chat_model": os.getenv("MODEL_CHOICE", ""),
    ...
    def _get_provider_base_url(self, provider: str, rag_settings: dict) -> str | None:
        """Get base URL for provider with environment variable fallback."""
        if provider == "ollama":
            # Priority: rag_settings → env vars → default
            base_url = rag_settings.get("LLM_BASE_URL")
            if not base_url:
