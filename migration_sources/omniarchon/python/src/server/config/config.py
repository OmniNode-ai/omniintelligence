...
class CorrelationConfig:
    ...
    # Enable/disable enhanced correlation system
    enabled: bool = True

    # Analysis mode configuration
    analysis_mode: str = "auto"  # auto, enhanced, basic, hybrid
    enhanced_threshold: float = 0.3  # Minimum rich data coverage for enhanced mode
...
def get_correlation_config() -> CorrelationConfig:
    """Load enhanced correlation configuration from environment variables."""
    ...
    def str_to_float(value: str | None, default: float) -> float:
        """Convert string environment variable to float with default."""
        if value is None:
            return default
        try:
            return float(value)
    ...
    def str_to_int(value: str | None, default: int) -> int:
        """Convert string environment variable to int with default."""
        if value is None:
            return default
        try:
            return int(value)
    ...
    return CorrelationConfig(
        enabled=str_to_bool(os.getenv("ENHANCED_CORRELATION_ENABLED", "true")),
        analysis_mode=os.getenv("CORRELATION_ANALYSIS_MODE", "auto"),
        enhanced_threshold=str_to_float(
            os.getenv("CORRELATION_ENHANCED_THRESHOLD"), 0.3
        ),
        technology_weight=str_to_float(os.getenv("CORRELATION_TECHNOLOGY_WEIGHT"), 0.4),
        architecture_weight=str_to_float(
            os.getenv("CORRELATION_ARCHITECTURE_WEIGHT"), 0.3
        ),
        rich_content_bonus=str_to_float(
            os.getenv("CORRELATION_RICH_CONTENT_BONUS"), 0.2
        ),
        fallback_timeout_seconds=str_to_int(
            os.getenv("CORRELATION_FALLBACK_TIMEOUT"), 30
        ),
        cache_analysis_results=str_to_bool(
            os.getenv("CORRELATION_CACHE_RESULTS", "true")
        ),
        batch_size=str_to_int(os.getenv("CORRELATION_BATCH_SIZE"), 5),
        max_context_documents=str_to_int(
            os.getenv("CORRELATION_MAX_CONTEXT_DOCS"), 100
        ),
        max_retries=str_to_int(os.getenv("CORRELATION_MAX_RETRIES"), 3),
        processing_interval=str_to_int(
            os.getenv("CORRELATION_PROCESSING_INTERVAL"), 60
        ),
        temporal_threshold_hours=str_to_int(
            os.getenv("CORRELATION_TEMPORAL_THRESHOLD_HOURS"), 72
        ),
        semantic_threshold=str_to_float(
            os.getenv("CORRELATION_SEMANTIC_THRESHOLD"), 0.3
        ),
        max_correlations_per_document=str_to_int(
            os.getenv("CORRELATION_MAX_CORRELATIONS"), 10
        ),
    )
