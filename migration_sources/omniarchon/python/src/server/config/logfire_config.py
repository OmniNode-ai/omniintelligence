...
def is_logfire_enabled() -> bool:
    ...
    _logfire_enabled = env_enabled in ("true", "1", "yes", "on")
...
def setup_logfire(
    ...
    if _logfire_enabled:
        ...
        if logfire_token:
            try:
                ...
                logging.error(
                    f"❌ Failed to configure Logfire: {e}. Using standard logging."
                )
                _logfire_enabled = False
        else:
    ...
    if not _logfire_enabled and LOGFIRE_AVAILABLE:
        try:
            ...
            logging.warning(
                f"⚠️  Warning: Could not configure Logfire in disabled mode: {e}"
            )
