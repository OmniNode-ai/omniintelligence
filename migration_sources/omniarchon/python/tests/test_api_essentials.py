...
def test_create_project(client, test_project, mock_database_client):
    ...
    mock_database_client.table.return_value.insert.return_value.execute.return_value.data = [
        {
            "id": "test-project-id",
...
def test_list_projects(client, mock_database_client):
    """Test listing projects endpoint exists and responds."""
    # Set up mock to return empty list (no projects)
    mock_database_client.table.return_value.select.return_value.execute.return_value.data = (
        []
    )
