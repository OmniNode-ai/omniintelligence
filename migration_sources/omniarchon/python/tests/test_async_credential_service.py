...
class TestAsyncCredentialService:
    ...
    async def test_get_credential_encrypted_value(self):
        ...
        with patch.object(
            ...
            credential_service._decrypt_value.assert_called_once_with(
                "encrypted_test_value"
            )

    @pytest.mark.asyncio
    async def test_get_credential_cache_not_initialized(self, mock_database_client):
        """Test getting credential when cache is not initialized"""
        mock_client, mock_table = mock_database_client
        ...
        with patch.object(
            credential_service, "_get_database_client", return_value=mock_client
        ):
    ...
    @pytest.mark.asyncio
    async def test_get_credential_not_found_in_db(self, mock_database_client):
    ...
    @pytest.mark.asyncio
    async def test_set_credential_new(self, mock_database_client):
    ...
    @pytest.mark.asyncio
    async def test_set_credential_encrypted(self, mock_database_client):
        ...
        with patch.object(
            ...
            with patch.object(
                credential_service, "_encrypt_value", return_value="encrypted_value"
            ):
                result = await set_credential(
                    "SECRET_KEY", "secret_value", is_encrypted=True
                )
                assert result is True

                # Should have encrypted the value
                credential_service._encrypt_value.assert_called_once_with(
                    "secret_value"
                )

    @pytest.mark.asyncio
    async def test_load_all_credentials(
        self, mock_database_client, sample_credentials_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_credentials_by_category(self, mock_database_client):
    ...
    @pytest.mark.asyncio
    async def test_get_active_provider_llm(self, mock_database_client):
        ...
        with patch.object(
            ...
            with patch.object(
                credential_service, "_decrypt_value", return_value="decrypted_key"
            ):
    ...
    @pytest.mark.asyncio
    async def test_get_active_provider_basic(self, mock_database_client):
    ...
    @pytest.mark.asyncio
    async def test_initialize_credentials(
        self, mock_database_client, sample_credentials_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_error_handling_database_failure(self, mock_database_client):
        """Test error handling when database fails"""
        mock_client, mock_table = mock_database_client

        # Mock database error
        mock_table.select().eq().execute.side_effect = Exception(
