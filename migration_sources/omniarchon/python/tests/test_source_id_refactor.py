...
class TestSourceIDGeneration:
    ...
    def test_unique_id_generation_basic(self):
        ...
        for url in test_urls:
            ...
            assert all(
                c in "0123456789abcdef" for c in source_id
            ), f"ID should be hex: {source_id}"

        # All IDs should be unique
        assert len(set(source_ids)) == len(
            source_ids
    ...
    def test_same_domain_different_ids(self):
        ...
        assert len(set(ids)) == len(
            ids
        ), "Same domain should generate different IDs for different URLs"
    ...
    def test_id_consistency(self):
        ...
        assert (
            len(set(ids)) == 1
        ), f"Same URL should always generate same ID, got: {set(ids)}"
    ...
    def test_url_normalization(self):
        ...
        assert (
            ids[0] == ids[2]
        ), "URLs with only domain case differences should generate same ID"
        assert (
            ids[0] != ids[1]
        ), "URLs with path case differences should generate different IDs"
    ...
    def test_concurrent_crawl_simulation(self):
        ...
        assert len(set(source_ids)) == len(
            source_ids
        ), "Concurrent crawls should generate unique IDs"
    ...
    def test_error_handling(self):
        ...
        for url in edge_cases:
            ...
            ), f"Should generate ID even for edge case: {url}"
            assert (
                len(source_id) == 16
            ), f"Edge case should still generate 16-char ID: {url}"
...
class TestDisplayNameExtraction:
    ...
    def test_github_display_names(self):
        ...
        for url, expected in test_cases:
            ...
            ), f"URL {url} should display as '{expected}', got '{display_name}'"
    ...
    def test_edge_case_display_names(self):
        ...
        for url, expected_contains in test_cases:
            display_name = handler.extract_display_name(url)
            assert (
                expected_contains in display_name or display_name == expected_contains
    ...
    def test_special_file_display_names(self):
        ...
        test_cases = [
            ...
            (
                ...
                "Example - Page.Html",
            ),  # Path gets added for single file
        ]
...
class TestIntegration:
    ...
    def test_full_source_creation_flow(self):
        ...
        assert (
            source_display_name == "GitHub - microsoft/typescript"
