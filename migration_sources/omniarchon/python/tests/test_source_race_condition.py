...
class TestSourceRaceCondition:
    ...
    def test_upsert_vs_insert_behavior(self):
        ...
        assert (
            "insert" not in methods_called
        ), "Should not use insert to avoid race conditions"
    ...
    def test_existing_source_uses_update(self):
        ...
        def track_update(data):
            ...
            return Mock(
                eq=Mock(return_value=Mock(execute=Mock(return_value=Mock(data=[]))))
            )
        ...
        assert (
            "upsert" not in methods_called
        ), "Should not use upsert for existing sources"
    ...
    def test_race_condition_with_delay(self):
        ...
        def delayed_check():
            ...
            if not source_created.is_set():
                ...
                result.data = [
                    {"source_id": "race_source", "title": "Existing", "metadata": {}}
                ]
            return result
