...
class TestURLHandler:
    ...
    def test_is_binary_file_with_query_params(self):
        ...
        assert (
            handler.is_binary_file("https://example.com/file.zip?version=1.0") is True
        )
        assert (
            handler.is_binary_file("https://example.com/document.pdf?download=true")
            is True
        )
    ...
    def test_is_binary_file_edge_cases(self):
        ...
        assert (
            handler.is_binary_file("https://example.com/jquery.min.js") is False
        )  # JS files might be crawlable

        # Real-world example from the error
        assert (
            handler.is_binary_file(
                "https://docs.crawl4ai.com/apps/crawl4ai-assistant/crawl4ai-assistant-v1.3.0.zip"
            )
            is True
        )
