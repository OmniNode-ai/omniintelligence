...
class TestAsyncLLMProviderService:
    ...
    async def test_get_llm_client_with_provider_override(self, mock_credential_service):
        ...
        with patch(
            ...
            with patch(
                ...
                mock_credential_service._get_provider_api_key.assert_called_once_with(
                    "openai"
                )
    ...
    @pytest.mark.asyncio
    async def test_get_llm_client_unsupported_provider_error(
    ...
    @pytest.mark.asyncio
    async def test_get_llm_client_with_unsupported_provider_override(
        self, mock_credential_service
    ):
        ...
        with patch(
            ...
            with pytest.raises(
                ValueError, match="Unsupported LLM provider: custom-unsupported"
            ):
    ...
    @pytest.mark.asyncio
    async def test_get_embedding_model_with_provider_override(
        self, mock_credential_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_embedding_model_custom_model_override(
        self, mock_credential_service
    ):
    ...
    async def test_get_embedding_model_error_fallback(self, mock_credential_service):
        ...
        mock_credential_service.get_active_provider.side_effect = Exception(
            "Database error"
        )
