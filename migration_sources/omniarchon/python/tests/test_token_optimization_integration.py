...
async def measure_response_size(
...
async def test_projects_endpoint():
    ...
    size_full, tokens_full = await measure_response_size(
        base_url, {"include_content": "true"}
    )
    if size_full > 0:
    ...
    # Test lightweight
    size_light, tokens_light = await measure_response_size(
        base_url, {"include_content": "false"}
    )
...
async def test_tasks_endpoint():
    ...
    size_full, tokens_full = await measure_response_size(
        base_url, {"exclude_large_fields": "false"}
    )
    if size_full > 0:
        print(f"Full content: {size_full:,} bytes | ~{tokens_full:,.0f} tokens")
    else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if projects and len(projects) > 0:
                    ...
                    print(
                        f"With content: {size_full:,} bytes | ~{tokens_full:,.0f} tokens"
                    )
                    ...
                    print(
                        f"Metadata only: {size_light:,} bytes | ~{tokens_light:,.0f} tokens"
                    )

                    # Calculate reduction if there are documents
                    if (
                        size_full > size_light and size_full > 500
