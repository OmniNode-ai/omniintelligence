...
class TestURLCanonicalization:
    ...
    def test_fragment_removal(self):
        ...
        assert (
            len(set(ids)) == 1
        ), "URLs with different fragments should generate same ID"
    ...
    def test_tracking_param_removal(self):
        ...
        for tracked_id in tracked_ids:
            assert (
                tracked_id == clean_id
            ), "URLs with tracking params should match clean URL"
    ...
    def test_query_param_sorting(self):
        ...
        assert (
            len(set(ids)) == 1
        ), "URLs with reordered query params should generate same ID"
    ...
    def test_default_port_removal(self):
        ...
        assert (
            len(set(http_ids)) == 1
        ), "HTTP URLs with/without :80 should generate same ID"
        ...
        assert (
            len(set(https_ids)) == 1
        ), "HTTPS URLs with/without :443 should generate same ID"
        ...
        assert (
            id1 != id2
        ), "URLs with different non-default ports should generate different IDs"
    ...
    def test_case_normalization(self):
        ...
        assert (
            len(set(ids)) == 1
        ), "URLs with different case in scheme/domain should generate same ID"
        ...
        assert (
            len(set(path_ids)) == 3
        ), "URLs with different path case should generate different IDs"
    ...
    def test_preserves_important_params(self):
        ...
        assert (
            id1 != id2
        ), "URLs with different important params should generate different IDs"
