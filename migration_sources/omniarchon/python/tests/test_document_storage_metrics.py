...
class TestDocumentStorageMetrics:
    ...
    async def test_avg_chunks_calculation_with_empty_docs(self):
        ...
        with patch(
            "src.server.services.crawling.document_storage_operations.safe_logfire_info"
        ) as mock_log:
            mock_log.side_effect = lambda msg: logged_messages.append(msg)

            with patch(
                "src.server.services.crawling.document_storage_operations.add_documents_to_database"
            ):
                ...
                assert (
                    "processed=3/5" in metrics_log
                ), "Should show 3 processed out of 5 total"
                assert "chunks=6" in metrics_log, "Should have 6 chunks total"
                assert (
                    "avg_chunks_per_doc=2.0" in metrics_log
                ), "Average should be 2.0 (6/3)"
    ...
    async def test_avg_chunks_single_doc(self):
        ...
        with patch(
            ...
            with patch(
                ...
                ), "Should show 1 processed out of 1 total"
    ...
    async def test_processed_count_accuracy(self):
        ...
        with patch(
            ...
            with patch(
                ...
                ), "Should have 3 chunks (one per processed doc)"
