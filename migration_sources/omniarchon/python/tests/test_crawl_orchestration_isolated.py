...
class MockCrawlOrchestrationService:
    ...
    async def _crawl_by_url_type(self, url: str, request: dict[str, Any]) -> tuple:
        ...
        if url.endswith(".txt"):
            return [
                {"url": url, "markdown": "Text content", "title": "Text File"}
            ], "text_file"
        elif "sitemap" in url:
            return [
                {
            ...
            return [
                {"url": url, "markdown": "Web content", "title": "Web Page"}
            ], "webpage"
...
class TestAsyncCrawlOrchestration:
    ...
    @pytest.mark.asyncio
    async def test_async_orchestrate_crawl_success(
        self, orchestration_service, sample_request
    ):
        """Test successful async orchestration"""
        result = await orchestration_service._async_orchestrate_crawl(
            sample_request, "task-123"
        )
    ...
    @pytest.mark.asyncio
    async def test_async_orchestrate_crawl_with_code_extraction(
    ...
    @pytest.mark.asyncio
    async def test_cancellation_during_orchestration(
        self, orchestration_service, sample_request
    ):
    ...
    def test_documentation_site_detection(self, orchestration_service):
        ...
        assert not orchestration_service._is_documentation_site(
    ...
    async def test_progress_callback_creation(self, orchestration_service):
        ...
        callback = await orchestration_service._create_crawl_progress_callback(
            "crawling"
        )
    ...
    @pytest.mark.asyncio
    async def test_main_orchestrate_crawl_entry_point(
        self, orchestration_service, sample_request
    ):
...
class TestAsyncBehaviors:
    ...
    async def test_no_blocking_operations(self):
        ...
        tasks = []
        for i in range(5):
            task = service._async_orchestrate_crawl(
