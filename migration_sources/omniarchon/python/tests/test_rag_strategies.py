...
with patch.dict(
    ...
    with patch(
        ...
        with patch("src.server.utils.get_database_client") as mock_supabase:
...
class TestRAGService:
    ...
    @pytest.fixture
    def rag_service(self, mock_database_client):
        """Create RAGService instance"""
        from src.server.services.search import RAGService

        return RAGService(supabase_client=mock_database_client)
    ...
    async def test_perform_rag_query(self, rag_service):
        ...
        with (
            ...
            patch.object(
                rag_service.reranking_strategy, "rerank_results"
            ) as mock_rerank,
        ):
...
class TestHybridSearchStrategy:
    ...
    @pytest.fixture
    def hybrid_strategy(self, mock_database_client):
        ...
        base_strategy = BaseSearchStrategy(mock_database_client)
        return HybridSearchStrategy(mock_database_client, base_strategy)
    ...
    def test_merge_search_results(self, hybrid_strategy):
        ...
        if merged:
            assert any(
                "Vector result" in str(r) or "Keyword result" in str(r) for r in merged
            )
...
class TestAgenticRAGStrategy:
    ...
    @pytest.fixture
    def agentic_strategy(self, mock_database_client):
        """Create AgenticRAGStrategy instance"""
        from src.server.services.search import AgenticRAGStrategy
        from src.server.services.search.base_search_strategy import BaseSearchStrategy

        base_strategy = BaseSearchStrategy(mock_database_client)
        return AgenticRAGStrategy(mock_database_client, base_strategy)
...
class TestRAGPerformance:
    ...
    async def test_concurrent_rag_queries(self, rag_service):
        ...
        with (
            ...
            tasks = [
                rag_service.perform_rag_query(query, match_count=3) for query in queries
            ]
