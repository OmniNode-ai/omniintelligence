...
class TestPortConfiguration:
    ...
    def test_service_discovery_requires_all_ports(self):
        ...
        with pytest.raises(
            ValueError, match="ARCHON_SERVER_PORT environment variable is required"
        ):
    ...
    def test_service_discovery_requires_mcp_port(self):
        ...
        with pytest.raises(
            ValueError, match="ARCHON_MCP_PORT environment variable is required"
        ):
            from src.server.config.service_discovery import ServiceDiscovery
    ...
    def test_service_discovery_requires_agents_port(self):
        ...
        with pytest.raises(
            ValueError, match="ARCHON_AGENTS_PORT environment variable is required"
        ):
            from src.server.config.service_discovery import ServiceDiscovery
    ...
    def test_config_requires_port_or_archon_mcp_port(self):
        ...
        with pytest.raises(
            ConfigurationError,
            match="PORT or ARCHON_MCP_PORT environment variable is required",
        ):
