...
from src.server.services.crawling.document_storage_operations import (
...
class TestCodeExtractionSourceId:
    ...
    async def test_code_extraction_uses_provided_source_id(self):
        ...
        async def mock_extract_blocks(
            crawl_results, source_id, progress_callback=None, start=0, end=100
        ):
        ...
        code_service._generate_code_summaries = AsyncMock(
            return_value=[{"summary": "Test code"}]
        )
        code_service._prepare_code_examples_for_storage = Mock(
            return_value=[
                {
                    "source_id": (
                        extracted_blocks[0]["source_id"] if extracted_blocks else None
                    )
                }
            ]
        )
        ...
        await code_service.extract_and_store_code_examples(
            crawl_results, url_to_full_document, correct_source_id, None, 0, 100
        )

        # Verify that extracted blocks use the correct source_id
        assert (
            len(extracted_blocks) > 0
        ), "Should have extracted at least one code block"

        for block in extracted_blocks:
            # Check that it's using the hash-based source_id, not the domain
            assert (
                block["source_id"] == correct_source_id
            ), f"Should use hash-based source_id '{correct_source_id}', not domain"
            assert (
                block["source_id"] != "docs.mem0.ai"
    ...
    async def test_document_storage_passes_source_id(self):
        ...
        doc_storage.code_extraction_service.extract_and_store_code_examples = (
            mock_extract
        )
        ...
        result = await doc_storage.extract_and_store_code_examples(
            crawl_results, url_to_full_document, source_id, None, 0, 100
        )
    ...
    async def test_no_domain_extraction_from_url(self):
        ...
        for url, expected_source_id in test_cases:
            ...
            await code_service.extract_and_store_code_examples(
                crawl_results, url_to_full_document, expected_source_id, None, 0, 100
            )
