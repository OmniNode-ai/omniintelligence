...
def test_optional_setting_returns_default(client, mock_database_client):
    ...
    with patch("server.api_routes.settings_api.credential_service", mock_service):
...
def test_unknown_credential_returns_404(client, mock_database_client):
...
def test_existing_credential_returns_normally(client, mock_database_client):
