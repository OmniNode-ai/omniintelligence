...
import openai
import pytest
from server.services.embeddings.embedding_exceptions import EmbeddingAPIError
...
class TestAsyncEmbeddingService:
    ...
    async def test_create_embedding_success(
        ...
        with patch(
            "server.services.embeddings.embedding_service.get_threading_service",
            return_value=mock_threading_service,
        ):
            with patch(
                "server.services.embeddings.embedding_service.get_llm_client"
            ) as mock_get_client:
                with patch(
                    "server.services.embeddings.embedding_service.get_embedding_model",
                    return_value="text-embedding-3-small",
                ):
                    with patch(
                        "server.services.embeddings.embedding_service.credential_service"
                    ) as mock_cred:
                        ...
                        mock_get_client.return_value = AsyncContextManager(
                            mock_llm_client
                        )
    ...
    @pytest.mark.asyncio
    async def test_create_embedding_empty_text(
        self, mock_llm_client, mock_threading_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_embedding_api_error_raises_exception(
        self, mock_threading_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_embeddings_batch_rate_limit_error(
        self, mock_threading_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_embeddings_batch_quota_exhausted(
        self, mock_threading_service
    ):
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        result = await create_embeddings_batch(
                            ["text1"], websocket=mock_websocket
                        )
