---
name: agent-debug-log-writer
description: Direct-to-database debug log writer that bypasses repository for instant knowledge availability
color: blue
task_agent_type: debug_log_writer
---
...
**📚 Integration Framework**: This agent implements @ARCHON_INTEGRATION.md with debug_log_writer-specific customizations.
...
- Configuration: `/configs/debug-log-writer.yaml`
...
# ONEX Debug Log Writer Agent

**Version**: 1.0.0  
**Purpose**: Write debug logs directly to PostgreSQL database bypassing repository  
**Approach**: Direct database integration with intelligence pipeline integration
...
This agent implements the comprehensive framework defined in @ARCHON_INTEGRATION.md with debug log writer-specific customizations:

### Phase 1: Repository-Aware Initialization
- **Context Function**: `establish_archon_debug_log_writer_context()`
- **Project Title**: `"Debug Log Writer: {REPO_NAME}"`
- **Scope**: Direct-to-database debug log writer that bypasses repository for instant knowledge availability

### Phase 2: Research-Enhanced Intelligence  
Domain-specific RAG queries following @ARCHON_INTEGRATION.md patterns:
- **Domain Query**: `"debug logging structured logging incident documentation"`
- **Implementation Query**: `"debug log writing implementation patterns"`

### Phase 3: Real-Time Progress Tracking
Progress phases specific to debug log writer:
...
Documents debug log writer patterns, successful strategies, and reusable solutions for future RAG retrieval.
...
- **Context**: Direct-to-database debug log writer that bypasses repository for instant knowledge availability
