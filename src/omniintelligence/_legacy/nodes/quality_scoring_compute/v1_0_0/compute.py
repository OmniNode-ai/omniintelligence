...
class PythonCodeAnalyzer:
    ...
    def analyze_maintainability(self) -> tuple[float, list[str]]:
        ...
        class MaintainabilityVisitor(ast.NodeVisitor):
            def __init__(self, lines: list[str]) -> None:
                ...
                self.max_nesting_depths: dict[str, int] = {}
            ...
            def _get_max_depth(self, node: ast.AST, current_depth: int) -> int:
    ...
    def analyze_testing(self) -> tuple[float, list[str]]:
        ...
        if is_test_file:
            ...
            if not test_visitor.test_functions:
                issues.append("No test functions found in test file")
                return 0.3, issues

            avg_assertions = test_visitor.assertions / len(test_visitor.test_functions)
            if avg_assertions < 1:
                issues.append("Test functions have few assertions")

            score = min(1.0, 0.5 + (avg_assertions / 5.0) * 0.3 + (0.2 if test_visitor.has_fixtures else 0.0))
            return score, issues
        else:
            # Check for testable structure
            class CodeVisitor(ast.NodeVisitor):
                def __init__(self) -> None:
                    self.public_functions: int = 0
                    self.functions_with_types: int = 0
            ...
            if self.tree is not None:
                code_visitor.visit(self.tree)

            if code_visitor.public_functions == 0:
                return 0.8, []  # No functions to test

            type_coverage = code_visitor.functions_with_types / code_visitor.public_functions
