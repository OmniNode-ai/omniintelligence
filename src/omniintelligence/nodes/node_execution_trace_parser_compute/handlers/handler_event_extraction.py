...
def detect_span_errors(span: SpanNodeDict) -> list[ErrorEventDict]:
...
def detect_timeout_errors(span: SpanNodeDict) -> list[ErrorEventDict]:
...
def extract_all_errors(
    ...
    errors.extend(detect_span_errors(span))

    # Detect log-level errors
    errors.extend(detect_log_errors(span, correlated_logs))

    # Detect timeout errors (may overlap with span errors)
    timeout_errors = detect_timeout_errors(span)
